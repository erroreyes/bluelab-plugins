...
void
TransientLib5::DetectTransients(WDL_TypedBuf<WDL_FFT_COMPLEX> *ioFftBuf,
                                BL_FLOAT threshold,
                                BL_FLOAT mix,
...
#define THRESHOLD_COEFF0 9.0
    ...
    WDL_TypedBuf<BL_FLOAT> &strippedMagns = mTmpBuf0;
    strippedMagns.Resize(bufSize);
    
    WDL_TypedBuf<BL_FLOAT> &transientMagns = mTmpBuf1;
...
void
TransientLib5::DetectTransientsSmooth(WDL_TypedBuf<WDL_FFT_COMPLEX> *ioFftBuf,
                                      BL_FLOAT threshold,
                                      BL_FLOAT mix,
                                      BL_FLOAT precision,
...
#define THRS_EPS_SMOOTH 1e-5
    ...
    WDL_TypedBuf<BL_FLOAT> &transients = mTmpBuf2;
    ...
    vector< vector< int > > &transToMagn = mTmpBuf3;
    ...
    WDL_TypedBuf<BL_FLOAT> &smoothedTransients = mTmpBuf5;
    ...
    //
    WDL_TypedBuf<BL_FLOAT> &newMagns = mTmpBuf6;
    newMagns = magns;
    for (int i = 0; i < bufSize; i++)
    {
...
void
TransientLib5::DetectTransientsSmooth2(WDL_TypedBuf<WDL_FFT_COMPLEX> *ioFftBuf,
                                       BL_FLOAT threshold,
                                       BL_FLOAT mix,
                                       BL_FLOAT precision,
                                       WDL_TypedBuf<BL_FLOAT> *outTransients,
...
{    
    ...
    WDL_TypedBuf<BL_FLOAT> &transients = mTmpBuf7;
    transients.Resize(bufSize);
    for (int i = 0; i < bufSize; i++)
        transients.Get()[i] = 0.0;
    ...
    // Map, to find the magn positions who have participated to
    // a given transient value
    vector< vector< int > > &transToMagn = mTmpBuf8;
    transToMagn.resize(bufSize);
    
    WDL_TypedBuf<BL_FLOAT> &magns = mTmpBuf9;
    magns.Resize(bufSize);
    ...
    // Smoother curve, for keeping non-transient parts
    WDL_TypedBuf<BL_FLOAT> &smoothedTransients = mTmpBuf10;
    GetSmoothedTransientsInterp(transients, &smoothedTransients, precision);
...
#define PRECISION1_COEFF_SMOOTH2 0.2
    ...
    // (by reversing the process)
    //
    WDL_TypedBuf<BL_FLOAT> &newMagns = mTmpBuf11;
    newMagns = magns;
    for (int i = 0; i < bufSize; i++)
    {
...
//
void
TransientLib5::ComputeTransientness(const WDL_TypedBuf<BL_FLOAT> &magns,
...
//
void
TransientLib5::ComputeTransientness2(const WDL_TypedBuf<BL_FLOAT> &magns,
                                     const WDL_TypedBuf<BL_FLOAT> &phases,
                                     const WDL_TypedBuf<BL_FLOAT> *prevPhases,
...
void
TransientLib5::ComputeTransientness3(const WDL_TypedBuf<BL_FLOAT> &magns,
                                     const WDL_TypedBuf<BL_FLOAT> &phases,
                                     const WDL_TypedBuf<BL_FLOAT> *prevPhases,
                                     BL_FLOAT freqAmpRatio,
                                     BL_FLOAT smoothFactor,
...
#define TRANS_COEFF_AMP_TR3 1.0
    ...
    WDL_TypedBuf<BL_FLOAT> &transientnessS = mTmpBuf14;
    transientnessS.Resize(phases.GetSize());
    BLUtils::FillAllZero(&transientnessS);
    
    WDL_TypedBuf<BL_FLOAT> &transientnessP = mTmpBuf15;
...
void
TransientLib5::ComputeTransientness4(const WDL_TypedBuf<BL_FLOAT> &magns,
                                     const WDL_TypedBuf<BL_FLOAT> &phases,
                                     const WDL_TypedBuf<BL_FLOAT> *prevPhases,
                                     BL_FLOAT freqAmpRatio,
                                     BL_FLOAT smoothFactor,
...
#define TRANS_COEFF_AMP_TR4 1.0
    ...
    WDL_TypedBuf<BL_FLOAT> &transientnessS = mTmpBuf17;
    transientnessS.Resize(phases.GetSize());
    BLUtils::FillAllZero(&transientnessS);
    
    WDL_TypedBuf<BL_FLOAT> &transientnessP = mTmpBuf18;
    transientnessP.Resize(phases.GetSize());
    BLUtils::FillAllZero(&transientnessP);
...
// (which made bad separation with sample rate 88200Hz)
void
TransientLib5::ComputeTransientness5(const WDL_TypedBuf<BL_FLOAT> &magns,
                                     const WDL_TypedBuf<BL_FLOAT> &phases,
                                     const WDL_TypedBuf<BL_FLOAT> *prevPhases,
                                     BL_FLOAT freqAmpRatio,
                                     BL_FLOAT smoothFactor,
...
#define USE_SAMPLE_RATE_COEFF_TR5 0 //1
    ...
    WDL_TypedBuf<BL_FLOAT> &transientnessS = mTmpBuf20;
    transientnessS.Resize(phases.GetSize());
    BLUtils::FillAllZero(&transientnessS);
    
    WDL_TypedBuf<BL_FLOAT> &transientnessP = mTmpBuf21;
    transientnessP.Resize(phases.GetSize());
    BLUtils::FillAllZero(&transientnessP);
...
void
TransientLib5::ComputeTransientness6(const WDL_TypedBuf<BL_FLOAT> &magns,
                                     const WDL_TypedBuf<BL_FLOAT> &phases,
                                     const WDL_TypedBuf<BL_FLOAT> *prevPhases,
                                     BL_FLOAT freqAmpRatio,
                                     BL_FLOAT smoothFactor,
...
#define TRANS_COEFF_AMP_TR6 1.0
    ...
    WDL_TypedBuf<BL_FLOAT> &transientnessS = mTmpBuf23;
    transientnessS.Resize(phases.GetSize());
    BLUtils::FillAllZero(&transientnessS);
    
    WDL_TypedBuf<BL_FLOAT> &transientnessP = mTmpBuf24;
    transientnessP.Resize(phases.GetSize());
    BLUtils::FillAllZero(&transientnessP);
...
void
TransientLib5::ComputeTransientnessMix(WDL_TypedBuf<BL_FLOAT> *magns,
                                       const WDL_TypedBuf<BL_FLOAT> &phases,
                                       const WDL_TypedBuf<BL_FLOAT> *prevPhases,
                                       //BL_FLOAT threshold,
...
#define KEEP_TRACK_OF_THRESHOLDED_MIX 1
    ...
    // Init
    WDL_TypedBuf<BL_FLOAT> &prevTransientness = mTmpBuf26;
    if (transientness != NULL)
    {
        prevTransientness = *transientness;
    }
    else
        prevTransientness.Resize(0);
    ...
    // Map, to find the magn positions who have participated to
    // a given transient value
    vector< vector< int > > &transToMagn = mTmpBuf28;
    ...
    //
    WDL_TypedBuf<BL_FLOAT> &newMagns = mTmpBuf29;
...
void
TransientLib5::ComputeTransientnessMix2(WDL_TypedBuf<BL_FLOAT> *magns,
                                        const WDL_TypedBuf<BL_FLOAT> &phases,
                                        const WDL_TypedBuf<BL_FLOAT> *prevPhases,
                                        //BL_FLOAT threshold,
                                        const WDL_TypedBuf<BL_FLOAT> &thresholds,
...
#define KEEP_TRACK_OF_THRESHOLDED_MIX2 1
    ...
    // Init
    WDL_TypedBuf<BL_FLOAT> &prevTransientness = mTmpBuf30;
    if (transientness != NULL)
    {
        prevTransientness = *transientness;
    }
    ...
    // Map, to find the magn positions who have participated to
    // a given transient value
    vector< vector< int > > &transToMagn = mTmpBuf32;
    transToMagn.resize(magns->GetSize());
    ...
    // NOTE: well checked: this method separates well the two parts
    //
    WDL_TypedBuf<BL_FLOAT> &newMagns = mTmpBuf33;
    ProcessMix(&newMagns, *magns, *transientness, transToMagn, thresholds, mix);
...
void
TransientLib5::ProcessMix(WDL_TypedBuf<BL_FLOAT> *newMagns,
...
void
TransientLib5::TransientnessToFreqDomain(WDL_TypedBuf<BL_FLOAT> *transientness,
...
void
TransientLib5::GetSmoothedTransients(const WDL_TypedBuf<BL_FLOAT> &transients,
...
void
TransientLib5::GetSmoothedTransientsInterp(const WDL_TypedBuf<BL_FLOAT> &transients,
...
#define FINE_SMOOTH_EXP_INT 8
  ...
  precision = std::pow(precision, 4.0);
    ...
    WDL_TypedBuf<BL_FLOAT> &smoothedTransientsRaw = mTmpBuf34;
    WDL_TypedBuf<BL_FLOAT> &smoothedTransientsFine = mTmpBuf35;
...
void
TransientLib5::NormalizeVolume(const WDL_TypedBuf<BL_FLOAT> &origin,
...
void
TransientLib5::NormalizeCurve(WDL_TypedBuf<BL_FLOAT> *ioCurve)
{
...
// Not used
BL_FLOAT
TransientLib5::GetThresholdedFactor(BL_FLOAT smoothTrans, BL_FLOAT threshold, BL_FLOAT mix)
{
    BL_FLOAT factor = 1.0;
...
void
TransientLib5::SmoothTransients(WDL_TypedBuf<BL_FLOAT> *transients,
                                BL_FLOAT smoothFactor)
{
    // Smooth if necessary
    if (smoothFactor > 0.0)
...
#define SMOOTH_FACTOR_TR 4.0
        ...
        WDL_TypedBuf<BL_FLOAT> &smoothTransients = mTmpBuf36;
...
void
TransientLib5::SmoothTransients2(WDL_TypedBuf<BL_FLOAT> *transients,
                                 BL_FLOAT smoothFactor,
                                 WDL_TypedBuf<BL_FLOAT> *smoothWin)
{
...
#define MAX_WIN_SIZE_TR2 512
    ...
    if (smoothFactor > 0.0)
    {
        ...
        WDL_TypedBuf<BL_FLOAT> &result = mTmpBuf37;
...
#if 0
void
TransientLib5::SmoothTransients3(WDL_TypedBuf<BL_FLOAT> *transients,
                                 BL_FLOAT smoothFactor)
{
...
void
TransientLib5::SmoothTransients3(WDL_TypedBuf<BL_FLOAT> *transients, int level)
{
    // Smooth if necessary
    if (level > 0)
    {
        WDL_TypedBuf<BL_FLOAT> &result = mTmpBuf38;
        BLUtils::SmoothDataPyramid(&result, *transients, level);
...
void
TransientLib5::SmoothTransients4(WDL_TypedBuf<BL_FLOAT> *transients)
{
...
void
TransientLib5::SmoothTransientsAdvanced(WDL_TypedBuf<BL_FLOAT> *transients,
                                        BL_FLOAT smoothFactor)
{
    // Smoother curve, for keeping non-transient parts
    WDL_TypedBuf<BL_FLOAT> &smoothedTransientness = mTmpBuf39;
