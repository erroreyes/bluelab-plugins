...
USTVumeter::USTVumeter(BL_FLOAT sampleRate)
{
    ...
    mPrevMillis = BLUtils::GetTimeMillis();
    mTextMillisFPS = 1000.0/TEXT_UPDATE_FPS;
}
...
bool
USTVumeter::NeedUpdateTexts()
{
    long int millis = BLUtils::GetTimeMillis();
    
    if (millis - mPrevMillis > mTextMillisFPS)
    {
        mPrevMillis = millis;
