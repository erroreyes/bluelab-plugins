...
template <typename FLOAT_TYPE>
void
BLUtils::LogScaleX(WDL_TypedBuf<FLOAT_TYPE> *values, FLOAT_TYPE factor)
{
    WDL_TypedBuf<FLOAT_TYPE> origValues = *values;
    ...
    for (int i = 0; i < valuesSize; i++)
    {
        FLOAT_TYPE t0 = factor*((FLOAT_TYPE)i)/valuesSize;
        
        FLOAT_TYPE t = (std::exp(t0) - 1.0)/(std::exp(factor) - 1.0);
...
template void BLUtils::LogScaleX(WDL_TypedBuf<float> *values, float factor);
template void BLUtils::LogScaleX(WDL_TypedBuf<double> *values, double factor);

template <typename FLOAT_TYPE>
FLOAT_TYPE
BLUtils::LogScale(FLOAT_TYPE value, FLOAT_TYPE factor)
{
    FLOAT_TYPE result = factor*value;
    
    result = (std::exp(result) - 1.0)/(std::exp(factor) - 1.0);
...
template float BLUtils::LogScale(float value, float factor);
template double BLUtils::LogScale(double value, double factor);

template <typename FLOAT_TYPE>
FLOAT_TYPE
BLUtils::LogScaleNorm(FLOAT_TYPE value, FLOAT_TYPE maxValue, FLOAT_TYPE factor)
{
...
template float BLUtils::LogScaleNorm(float value, float maxValue, float factor);
template double BLUtils::LogScaleNorm(double value, double maxValue, double factor);
