...
#define OPTIM_FAST_LOG2 0 //1
#define OPTIM_FAST_MATH 1
...
#else
    if (sizeof(FLOAT_TYPE) == 4)
        return expf_fast(((FLOAT_TYPE)BL_IAMP_DB) * dB);
    else
        return exp_fast(((FLOAT_TYPE)BL_IAMP_DB) * dB);
#endif
}
...
#define AMPTODB_COEFF ((float)BL_AMP_DB)*LN2
    return AMPTODB_COEFF*fastlog2(fabsf((float)dB));
#elif OPTIM_FAST_MATH
    if (sizeof(FLOAT_TYPE) == 4)
        return ((FLOAT_TYPE)BL_AMP_DB)*logf_fast(std::fabs(dB));
    else
        return ((FLOAT_TYPE)BL_AMP_DB)*log_fast(std::fabs(dB));
#else
