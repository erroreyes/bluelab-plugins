                                   ...
                                   const WDL_TypedBuf<BL_GUI_FLOAT> &yValues)
{
    ...
    //IPluginBase::IMutexLock lock(mPlug); // ??
...
// #bl-iplug2
void
GraphControl11::CustomDrawersPreDraw(/*IPlugBase::IMutexLock *lock*/)
{
    int width = this->mRECT.W();
    int height = this->mRECT.H();
...
void
GraphControl11::CustomDrawersPostDraw(/*IPlugBase::IMutexLock *lock*/)
{
    int width = this->mRECT.W();
    int height = this->mRECT.H();
...
void
GraphControl11::CreateImageDisplay(BL_GUI_FLOAT left, BL_GUI_FLOAT top, BL_GUI_FLOAT right, BL_GUI_FLOAT bottom,
...
#if 0
void
GraphControl11::SetCurveValuePoint(int curveNum, BL_GUI_FLOAT x, BL_GUI_FLOAT y)
{
    if (curveNum >= mNumCurves)
        return;
...
#define OFFSET lineWidth
...
#endif
    {
        ...
        nvgLineTo(mVg, x, originYf/*0*/);
        nvgLineTo(mVg, prevX, originYf/*0*/);
...
#define FILL_RECTS 1
    ...
    for (int i = 0; i < curve->mXValues.GetSize(); i ++)
    {
        ...
        nvgBeginPath(mVg);
#endif
        nvgRect(mVg, x, yf, pointSize, pointSize);
...
#if 0
void
GraphControl11::DrawPointCurveWeights(GraphCurve4 *curve)
{
#if FIX_UNDEFINED_CURVES
    bool curveUndefined = IsCurveUndefined(curve->mXValues, curve->mYValues, 1);
    if (curveUndefined)
...
GraphControl11::Draw(IGraphics &graphics)
{
    ...
    /*nvgTranslate(mVg, 0.0, -735);
    nvgScale(mVg, 1.0, -1.0);
    nvgTranslate(mVg, 0.0, 735 + this->mRECT.H());*/
    ...
    //unsigned char clearColor[4] = { (unsigned char)(mClearColor[0]*255),
    //                                (unsigned char)(mClearColor[1]*255),
    //                                (unsigned char)(mClearColor[2]*255),
    //                                (unsigned char)(mClearColor[3]*255) };
    ...
    //nvgBeginFrame(mVg, width, height, 1.0); // NEW
...
/*
void
GraphControl11::UpdateBackgroundImage()
{
    if (mBgImage == NULL)
        return;
...
/*
void
GraphControl11::UpdateOverlayImage()
{
    if (mOverImage == NULL)
        return;
...
#if 0
    TODO if re-used: manage GRAPH_CONTROL_FLIP_Y
