                                ...
                                WDL_TypedBuf<BL_FLOAT> *resultMask)
{
    ...
    BLDebug::DumpData("mask1.txt", masks[0]);
...
void
RebalanceMaskProcessor::SetVocalContrast(BL_FLOAT contrast)
{
    mSensitivities[0] = contrast;
}

void
RebalanceMaskProcessor::SetBassContrast(BL_FLOAT contrast)
{
    mSensitivities[1] = contrast;
}

void
RebalanceMaskProcessor::SetDrumsContrast(BL_FLOAT contrast)
{
    mSensitivities[2] = contrast;
}

void
RebalanceMaskProcessor::SetOtherContrast(BL_FLOAT contrast)
{
    mSensitivities[3] = contrast;
}

void
RebalanceMaskProcessor::ApplyContrass(BL_FLOAT masks[NUM_STEM_SOURCES])
{
    for (int i = 0; i < NUM_STEM_SOURCES; i++)
    {
        ApplyMaskContrast(masks[i],  mContrasts[i]);
    }
}

void
RebalanceMaskProcessor::
ApplyMaskContrast(WDL_TypedBuf<BL_FLOAT> masks[NUM_STEM_SOURCES],
                  BL_FLOAT contrast)
{
