...
SineSynth2::Partial::Partial(const Partial &other)
{
    mFreq = other.mFreq;
    mAmpDB = other.mAmpDB;
    mPhase = other.mPhase;
...
SineSynth2::SineSynth2(int bufferSize, BL_FLOAT sampleRate, int overlapping)
{
...
void
SineSynth2::Reset(int bufferSize, BL_FLOAT sampleRate, int overlapping)
{
    mPartials.clear();
    mPrevPartials.clear();
...
#if 0 // ORIGIN
void
SineSynth2::ComputeSamples(WDL_TypedBuf<BL_FLOAT> *samples)
{
    samples->Resize(mBufferSize);
    Utils::FillAllZero(samples);
...
#if 0
void
SineSynth2::ComputeSamplesWin(WDL_TypedBuf<BL_FLOAT> *samples)
{
#if !COMPUTE_SAS_SAMPLES_TABLE
...
void
SineSynth2::ComputeSamplesPartials(WDL_TypedBuf<BL_FLOAT> *samples)
{
    samples->Resize(mBufferSize);
...
//
void
SineSynth2::ComputeSamplesSAS5(WDL_TypedBuf<BL_FLOAT> *samples)
{
    samples->Resize(mBufferSize);
...
#if 0
void
SineSynth2::ComputeSamplesSASTable(WDL_TypedBuf<BL_FLOAT> *samples)
{
    samples->Resize(mBufferSize);
    Utils::FillAllZero(samples);
    ...
    while((freq < mSampleRate/2.0) && (partialIndex < SYNTH_MAX_NUM_PARTIALS))
    {
        if (freq > SYNTH_MIN_FREQ)
        {
            ...
            if (freq >= SYNTH_MIN_FREQ)
            {
                ...
                Utils::AddValues(samples, freqBuffer);
            }
...
#if 0
void
SineSynth2::ComputeSamplesSASTable2(WDL_TypedBuf<BL_FLOAT> *samples)
{
    samples->Resize(mBufferSize);
    Utils::FillAllZero(samples);
...
#if 0
bool
SineSynth2::FindPartial(BL_FLOAT freq)
{
...
#if 0
int
SineSynth2::FindPrevPartialIdx(int currentPartialIdx)
{
...
int
SineSynth2::FindPartialFromIdx(int partialIdx)
{
...
#if 0
void
SineSynth2::GetPartial(Partial *result, int index, BL_FLOAT t)
{
