                             ...
                             int width, int height)
{
    ...
    for (int i = 0; i < points->size(); i++)
    {
        BL_FLOAT targetIIdx = i*iCoeff;
        if (targetIIdx > slices.size() - 1)
            targetIIdx = slices.size() - 1;
        
#if 0
        vector<Point> slice0 = slices[targetIIdx];
        if (iCoeff > 1.0)
        {
            // If we decimate, interpolate, so we are sure we won't have jitter/jumps
            // in the lines.
            if (targetIIdx + 1 < slices.size())
            {
                BL_FLOAT t = targetIIdx - (int)targetIIdx;
                
                const vector<Point> &s0 = slices[targetIIdx];
                const vector<Point> &s1 = slices[targetIIdx + 1];
                
                for (int k = 0; k < slice0.size(); k++)
                {
                    Point &p = slice0[k];
                    ...
                    p.mX = (1.0 - t)*p0.mX + t*p1.mX;
                    p.mY = (1.0 - t)*p0.mY + t*p1.mY;
                    p.mZ = (1.0 - t)*p0.mZ + t*p1.mZ;
                    
                    p.mSize = (1.0 - t)*p0.mSize + t*p1.mSize;
                }
            }
        }
#endif
        ...
        if (mDensePointsFlag)
        {
            ...
            //BL_FLOAT jCoeff = ((BL_FLOAT)slice0.size())/densityNumSlicesJ;
            for (int j = 0; j < newPoints.size(); j++)
            {
                ...
                //if (targetJIdx > (int)slice0.size() - 1)
                //    targetJIdx = (int)slice0.size() - 1;
                //const Point &p = slice0[targetJIdx];
