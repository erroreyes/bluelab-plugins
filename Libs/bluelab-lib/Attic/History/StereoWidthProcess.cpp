...
#define TEMPORAL_DIST_SMOOTH_COEFF 0.9 //0.5 //0.9
...
void
StereoWidthProcess::SetWidthChange(BL_FLOAT widthChange)
{
...
#define EPS 1e-15
    ...
    if (mDisplayMode == SIMPLE)
    {
        ...
        BLUtils::MultValues(&mWidthValuesX, -1.0);
        ...
        mFakeStereo)
    {
        ...
        BLUtils::FillAllValue(&mWidthValuesX, 0.0);
        BLUtils::FillAllValue(&mWidthValuesY, 0.0);
        
        BLUtils::FillAllValue(&mColorWeights, 0.0);
    }
...
void
StereoWidthProcess::GetWidthValues(WDL_TypedBuf<BL_FLOAT> *xValues,
...
void
StereoWidthProcess::SetDisplayMode(enum DisplayMode mode)
{
    mDisplayMode = mode;
}

void
StereoWidthProcess::ApplyWidthChange(WDL_TypedBuf<BL_FLOAT> *ioMagnsL,
                                     ...
                                     WDL_TypedBuf<BL_FLOAT> *outSourceThetas)
{
    ...
    // Retrieve magn and phase from new changed microphone distance
...
void
StereoWidthProcess::Stereoize(WDL_TypedBuf<BL_FLOAT> *ioPhasesL,
                              WDL_TypedBuf<BL_FLOAT> *ioPhasesR)

{
    ...
    BL_FLOAT widthChange =
        BLUtils::FactorToDivFactor(mWidthChange, MAX_WIDTH_CHANGE_STEREOIZE);
...
void
StereoWidthProcess::MagnPhasesToSourcePos(WDL_TypedBuf<BL_FLOAT> *outSourceRs,
                                          WDL_TypedBuf<BL_FLOAT> *outSourceThetas,
...
bool
StereoWidthProcess::MagnPhasesToSourcePos(BL_FLOAT *sourceR, BL_FLOAT *sourceTheta,
...
#define EPS 1e-8
  ...
  if (std::fabs(freq - DEBUG_DRAWER_DISPLAY_FREQ) < EPS)
    ...
    *sourceTheta = 0.0;
    ...
    //BLDebug::AppendValue("a0.txt", a); // OK
    
    //BLDebug::AppendValue("b0.txt", b); // OK
    
    //BLDebug::AppendValue("cosTheta0.txt", cosTheta); // OK
    
    //BLDebug::AppendValue("G0.txt", G); // OK
    
    //BLDebug::AppendValue("phaseDiff0.txt", phaseDiff);
    ...
    //BLDebug::AppendValue("theta0.txt", theta); // OK
    ...
    *sourceTheta = theta;
    ...
    //BLDebug::AppendValue("r0.txt", r); // OK
    
    return true;
}

void
StereoWidthProcess::SourcePosToMagnPhases(const WDL_TypedBuf<BL_FLOAT> &sourceRs,
...
bool
StereoWidthProcess::SourcePosToMagnPhases(BL_FLOAT sourceR, BL_FLOAT sourceTheta,
                                          BL_FLOAT d,
...
#define EPS 1e-15
    ...
    BL_FLOAT sX = sourceR*std::cos(sourceTheta);
    BL_FLOAT sY = -sourceR*std::sin(sourceTheta);
...
bool
StereoWidthProcess::SourcePosToMagnPhases2(BL_FLOAT sourceR, BL_FLOAT sourceTheta,
                                           BL_FLOAT d,
                                           BL_FLOAT *ioMagnL, BL_FLOAT *ioMagnR,
                                           BL_FLOAT *ioPhaseL, BL_FLOAT *ioPhaseR,
                                           BL_FLOAT freq,
...
#define EPS 1e-8
    ...
    *ioPhaseR = *ioPhaseL + phaseDiff;
    ...
    //BL_FLOAT middlePhase = (*ioPhaseL + *ioPhaseR)/2.0;
...
BL_FLOAT
StereoWidthProcess::ComputeFactor(BL_FLOAT normVal, BL_FLOAT maxVal)
{
    BL_FLOAT res = 0.0;
    if (normVal < 0.0)
        res = normVal + 1.0;
...
void
StereoWidthProcess::ComputeTimeDelays(WDL_TypedBuf<BL_FLOAT> *timeDelays,
                                      const WDL_TypedBuf<BL_FLOAT> &phasesL,
                                      const WDL_TypedBuf<BL_FLOAT> &phasesR)
{
    if (phasesL.GetSize() != phasesR.GetSize())
...
void
StereoWidthProcess::ModifyPhase(BL_FLOAT *phaseL,
                                BL_FLOAT *phaseR,
                                BL_FLOAT factor)
{
    BL_FLOAT phaseDiff = *phaseR - *phaseL;
...
void
StereoWidthProcess::TemporalSmoothing(WDL_TypedBuf<BL_FLOAT> *sourceRs,
                                      WDL_TypedBuf<BL_FLOAT> *sourceThetas)
{
    // Left
...
void
StereoWidthProcess::SpatialSmoothing(WDL_TypedBuf<BL_FLOAT> *sourceRs,
                                     WDL_TypedBuf<BL_FLOAT> *sourceThetas)
{
...
void
StereoWidthProcess::NormalizeRs(WDL_TypedBuf<BL_FLOAT> *sourceRs,
                                bool discardClip)
{
    // 100 m
...
void
StereoWidthProcess::ComputeColorWeightsMagns(WDL_TypedBuf<BL_FLOAT> *outWeights,
                                             const WDL_TypedBuf<BL_FLOAT> &magnsL,
                                             const WDL_TypedBuf<BL_FLOAT> &magnsR)
{
    // Mono
...
void
StereoWidthProcess::ComputeColorWeightsFreqs(WDL_TypedBuf<BL_FLOAT> *outWeights,
                                             const WDL_TypedBuf<BL_FLOAT> &freqs)
{
...
int
StereoWidthProcess::SecondOrderEqSolve(BL_FLOAT a, BL_FLOAT b, BL_FLOAT c, BL_FLOAT res[2])
{
