...
GraphControl7::GraphControl7(IPlugBase *pPlug, IRECT pR, int paramIdx,
...
void
GraphControl7::Resize(int numCurveValues)
{
    mNumCurveValues = numCurveValues;
...
int
GraphControl7::GetNumCurveValues()
{
    return mNumCurveValues;
}

bool
GraphControl7::IsDirty()
{
    return mDirty;
}

const LICE_IBitmap *
GraphControl7::DrawGL()
{
#if PROFILE_GRAPH
    mDebugTimer.Start();
#endif
    ...
    if (mDebugCount++ % 100 == 0)
    {
        ...
        fprintf(stderr, "GraphControl7 - profile: %ld\n", t);
        
        char debugMessage[512];
        sprintf(debugMessage, "GraphControl7 - profile: %ld", t);
...
void
GraphControl7::AddHAxis(char *data[][2], int numData, int axisColor[4], int axisLabelColor[4])
{
...
void
GraphControl7::ReplaceHAxis(char *data[][2], int numData)
{
    int axisColor[4];
...
void
GraphControl7::AddVAxis(char *data[][2], int numData, int axisColor[4], int axisLabelColor[4], BL_FLOAT offset)
{
...
void
GraphControl7::AddVAxis(char *data[][2], int numData, int axisColor[4], int axisLabelColor[4],
...
void
GraphControl7::SetXScale(bool dbFlag, BL_FLOAT minX, BL_FLOAT maxX)
{
    mXdBScale = dbFlag;
...
void
GraphControl7::SetAutoAdjust(bool flag, BL_FLOAT smoothCoeff)
{
...
void
GraphControl7::SetYScaleFactor(BL_FLOAT factor)
{
...
void
GraphControl7::SetClearColor(int r, int g, int b, int a)
{
...
void
GraphControl7::SetCurveColor(int curveNum, int r, int g, int b)
{
...
void
GraphControl7::SetCurveAlpha(int curveNum, BL_FLOAT alpha)
{
    if (curveNum >= mNumCurves)
        return;
...
void
GraphControl7::SetCurveLineWidth(int curveNum, BL_FLOAT lineWidth)
{
    if (curveNum >= mNumCurves)
        return;
...
void
GraphControl7::SetCurveSmooth(int curveNum, bool flag)
{
    if (curveNum >= mNumCurves)
        return;
...
void
GraphControl7::SetCurveFill(int curveNum, bool flag)
{
    if (curveNum >= mNumCurves)
        return;
...
void
GraphControl7::SetCurveFillAlpha(int curveNum, BL_FLOAT alpha)
{
    if (curveNum >= mNumCurves)
        return;
...
void
GraphControl7::SetCurveSingleValueH(int curveNum, bool flag)
{
    if (curveNum >= mNumCurves)
        return;
...
void
GraphControl7::SetCurveSingleValueV(int curveNum, bool flag)
{
    if (curveNum >= mNumCurves)
        return;
...
void
GraphControl7::SetSpectrogram(BLSpectrogram *spectro,
...
void
GraphControl7::ShowSpectrogram(bool flag)
{
    mShowSpectrogram = flag;
}

void
GraphControl7::UpdateSpectrogram(bool updateData)
{
...
void
GraphControl7::SetSpectrogramZoomX(BL_FLOAT zoomX)
{
    mSpectrogramZoomX = zoomX;
}

void
GraphControl7::SetSpectrogramZoomY(BL_FLOAT zoomY)
{
    mSpectrogramZoomY = zoomY;
}

void
GraphControl7::InitGL()
{
    if (mGLInitialized)
        return;
...
void
GraphControl7::DisplayCurveDescriptions()
{
#define DESCR_X 40.0
...
void
GraphControl7::AddAxis(GraphAxis *axis, char *data[][2], int numData, int axisColor[4],
                       int axisLabelColor[4],
...
void
GraphControl7::SetCurveDescription(int curveNum, const char *description, int descrColor[4])
{
    if (curveNum >= mNumCurves)
        return;
...
void
GraphControl7::ResetCurve(int curveNum, BL_FLOAT val)
{
    IPlugBase::IMutexLock lock(mPlug);
...
void
GraphControl7::SetCurveYScale(int curveNum, bool dbFlag, BL_FLOAT minY, BL_FLOAT maxY)
{
    if (curveNum >= mNumCurves)
        return;
...
void
GraphControl7::CurveFillAllValues(int curveNum, BL_FLOAT val)
{
...
void
GraphControl7::SetCurveValues(int curveNum, const WDL_TypedBuf<BL_FLOAT> *values)
{
    // Must lock otherwise with DATA_STEP, curve will jerk
    IPlugBase::IMutexLock lock(mPlug);
...
void
GraphControl7::SetCurveValues2(int curveNum, const WDL_TypedBuf<BL_FLOAT> *values)
{
    // Must lock otherwise with DATA_STEP, curve will jerk
    IPlugBase::IMutexLock lock(mPlug);
...
void
GraphControl7::SetCurveValuesDecimate(int curveNum,
...
void
GraphControl7::SetCurveValue(int curveNum, BL_FLOAT t, BL_FLOAT val)
{
...
void
GraphControl7::SetCurveSingleValueH(int curveNum, BL_FLOAT val)
{
    // Must lock otherwise with DATA_STEP, curve will jerk
    IPlugBase::IMutexLock lock(mPlug);
...
void
GraphControl7::SetCurveSingleValueV(int curveNum, BL_FLOAT val)
{
    // Must lock otherwise with DATA_STEP, curve will jerk
    IPlugBase::IMutexLock lock(mPlug);
...
void
GraphControl7::PushCurveValue(int curveNum, BL_FLOAT val)
{
    IPlugBase::IMutexLock lock(mPlug);
...
void
GraphControl7::DrawAxis(bool lineLabelFlag)
{
    if (mHAxis != NULL)
...
void
GraphControl7::DrawAxis(GraphAxis *axis, bool horizontal, bool lineLabelFlag)
{
    nvgSave(mVg);
...
void
GraphControl7::DrawLineCurve(GraphCurve4 *curve)
{
#if CURVE_DEBUG
...
#if CURVE_DEBUG
    fprintf(stderr, "GraphControl7::DrawLineCurve - num points: %d\n", numPointsDrawn);
#endif
}
...
void
GraphControl7::DrawFillCurve(GraphCurve *curve)
{    
...
void
GraphControl7::DrawFillCurve(GraphCurve4 *curve)
{
#if CURVE_DEBUG
    int numPointsDrawn = 0;
#endif
...
#if CURVE_DEBUG
    fprintf(stderr, "GraphControl7::DrawFillCurve - num points: %d\n", numPointsDrawn);
#endif
}
#endif

void
GraphControl7::DrawLineCurveSVH(GraphCurve4 *curve)
{
...
void
GraphControl7::DrawFillCurveSVH(GraphCurve4 *curve)
{
    if (curve->mYValues.GetSize() == 0)
        return;
...
void
GraphControl7::DrawLineCurveSVV(GraphCurve4 *curve)
{
...
// (only, for the moment)
void
GraphControl7::DrawFillCurveSVV(GraphCurve4 *curve)
{
    // Finally, take the Y value
    // We will have to care about the curve Y scale !
    if (curve->mYValues.GetSize() == 0)
...
BL_FLOAT
GraphControl7::MillisToPoints(long long int elapsed, int sampleRate, int numSamplesPoint)
{
...
void
GraphControl7::InitFont(const char *fontPath)
{
#ifndef WIN32
...
void
GraphControl7::DrawText(BL_FLOAT x, BL_FLOAT y, BL_FLOAT fontSize,
...
BL_FLOAT
GraphControl7::ConvertY(GraphCurve4 *curve, BL_FLOAT val, BL_FLOAT height)
{
    BL_FLOAT y = val;
    if (y != GRAPH_VALUE_UNDEFINED)
    {
...
void
GraphControl7::SetCurveDrawStyle(GraphCurve4 *curve)
{
    nvgStrokeWidth(mVg, curve->mLineWidth);
...
void
GraphControl7::DoUpdateSpectrogram()
{    
