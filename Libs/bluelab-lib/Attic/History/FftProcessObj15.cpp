...
void
ProcessObjChannel::SetLatency(int latency)
{
    mLatency = latency;
    mCurrentLatency = mLatency;
}
...
ProcessObjChannel::GetResult(WDL_TypedBuf<BL_FLOAT> *output, int numRequested)
{
        ...
        // (as required when there is latency and data is not yet available)
    {
      ...
      // buffer size 2000, width=100%, bypass FftProcessObj15::ProcessFftBuffer()
      // => crackles
...
void
FftProcessObj15::SetLatency(int latency)
{
    for (int i = 0; i < mChannels.size(); i++)
        mChannels[i]->SetLatency(latency);
}

int
FftProcessObj15::GetLatency()
{
    if (mChannels.empty())
        return mBufferSize;
