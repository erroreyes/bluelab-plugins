...
USTVectorscope2::USTVectorscope2()
{
    mIsEnabled = false;
...
void
USTVectorscope2::SetEnabled(bool flag)
{
    mIsEnabled = flag;
    ...
    mPrevPolarSampleLevels.Resize(0);
    mPrevPolarSampleLevelsMax.Resize(0);
}

void
USTVectorscope2::Reset()
{
    mPrevPolarSampleLevels.Resize(0);
    mPrevPolarSampleLevelsMax.Resize(0);
}
...
void
USTVectorscope2::SetMode(Mode mode)
{
    // TODO
    mMode = mode;
...
int
USTVectorscope2::GetNumCurves(int graphNum)
{
    if (graphNum == 0)
...
int
USTVectorscope2::GetNumPoints(int graphNum)
{
    return NUM_POINTS;
}

void
USTVectorscope2::SetGraphs(GraphControl11 *graph0,
...
void
USTVectorscope2::AddSamples(vector<WDL_TypedBuf<BL_FLOAT> > samples)
{
    if (samples.size() != 2)
        return;
    ...
    if (mMode == POLAR_LEVEL)
    {
        if (mGraphs[1] != NULL)
        {
            ...
            //BLUtils::AmpToDBNorm(&samplesIn[0], 1e-10, -120.0);
            //BLUtils::AmpToDBNorm(&samplesIn[1], 1e-10, -120.0);
            //BLUtils::DBToAmpNorm(&samplesIn[0], 1e-10, -120.0);
            //BLUtils::DBToAmpNorm(&samplesIn[1], 1e-10, -120.0);
            
            WDL_TypedBuf<BL_FLOAT> polarSamples[2];
            USTProcess::ComputePolarLevel(samplesIn/*mSamples*/,
                                          polarSamples, &mPrevPolarSampleLevels);
            
            mGraphs[1]->SetCurveValuesPoint(CURVE_POINTS0,
                                            polarSamples[0], polarSamples[1]);
            
            WDL_TypedBuf<BL_FLOAT> polarSamplesMax[2];
            USTProcess::ComputePolarLevel(samplesIn/*mSamples*/, polarSamplesMax,
                                          &mPrevPolarSampleLevelsMax, true);
...
void
USTVectorscope2::SetCurveStyle(GraphControl11 *graph,
                               int curveNum,
