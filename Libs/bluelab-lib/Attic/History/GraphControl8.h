...
//
// GraphControl8 : for BLSpectrogram2
...
public:
    ...
    virtual void PreDraw(NVGcontext *, int width, int height) {}
...
class GraphControl8 : public IControl
{
public:
    GraphControl8(IPlugBase *pPlug, IRECT p, int paramIdx,
                 int numCurves, int numCurveValues, const char *fontPath);
    
    virtual ~GraphControl8();
    ...
    void AddVAxis(char *data[][2], int numData,
                  int axisColor[4], int axisLabelColor[4],
                  BL_FLOAT offset = 0.0, BL_FLOAT offsetX = 0.0); // Tmp offset hack
    ...
    // Spectrogram
    void SetSpectrogram(BLSpectrogram2 *spectro,
