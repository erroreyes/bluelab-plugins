...
void
GraphControl6::Resize(int numCurveValues)
{
    mNumCurveValues = numCurveValues;
...
GraphControl6::DrawGL()
{
    ...
    if (mDebugCount++ % 100 == 0)
    {
        ...
        sprintf(debugMessage, "GraphControl6 - profile: %ld", t);
...
void
GraphControl6::AddVAxis(char *data[][2], int numData, int axisColor[4], int axisLabelColor[4], BL_FLOAT offset)
{
...
void
GraphControl6::AddVAxis(char *data[][2], int numData, int axisColor[4], int axisLabelColor[4],
...
void
GraphControl6::SetXScale(bool dbFlag, BL_FLOAT minX, BL_FLOAT maxX)
{
    mXdBScale = dbFlag;
...
void
GraphControl6::SetAutoAdjust(bool flag, BL_FLOAT smoothCoeff)
{
...
void
GraphControl6::SetYScaleFactor(BL_FLOAT factor)
{
...
void
GraphControl6::SetCurveAlpha(int curveNum, BL_FLOAT alpha)
{
    if (curveNum >= mNumCurves)
        return;
...
void
GraphControl6::SetCurveLineWidth(int curveNum, BL_FLOAT lineWidth)
{
    if (curveNum >= mNumCurves)
        return;
...
void
GraphControl6::SetCurveFillAlpha(int curveNum, BL_FLOAT alpha)
{
    if (curveNum >= mNumCurves)
        return;
...
void
GraphControl6::ResetCurve(int curveNum, BL_FLOAT val)
{
    IPlugBase::IMutexLock lock(mPlug);
...
void
GraphControl6::SetCurveYScale(int curveNum, bool dbFlag, BL_FLOAT minY, BL_FLOAT maxY)
{
    if (curveNum >= mNumCurves)
        return;
...
void
GraphControl6::CurveFillAllValues(int curveNum, BL_FLOAT val)
{
...
void
GraphControl6::SetCurveValues(int curveNum, const WDL_TypedBuf<BL_FLOAT> *values)
{
    // Must lock otherwise with DATA_STEP, curve will jerk
    IPlugBase::IMutexLock lock(mPlug);
...
void
GraphControl6::SetCurveValues2(int curveNum, const WDL_TypedBuf<BL_FLOAT> *values)
{
    // Must lock otherwise with DATA_STEP, curve will jerk
    IPlugBase::IMutexLock lock(mPlug);
...
void
GraphControl6::SetCurveValue(int curveNum, BL_FLOAT t, BL_FLOAT val)
{
...
void
GraphControl6::SetCurveSingleValueH(int curveNum, BL_FLOAT val)
{
    // Must lock otherwise with DATA_STEP, curve will jerk
    IPlugBase::IMutexLock lock(mPlug);
...
void
GraphControl6::SetCurveSingleValueV(int curveNum, BL_FLOAT val)
{
    // Must lock otherwise with DATA_STEP, curve will jerk
    IPlugBase::IMutexLock lock(mPlug);
...
void
GraphControl6::PushCurveValue(int curveNum, BL_FLOAT val)
{
    IPlugBase::IMutexLock lock(mPlug);
...
void
GraphControl6::DrawText(BL_FLOAT x, BL_FLOAT y, BL_FLOAT fontSize,
...
BL_FLOAT
GraphControl6::ConvertY(GraphCurve4 *curve, BL_FLOAT val, BL_FLOAT height)
{
    BL_FLOAT y = val;
    if (y != GRAPH_VALUE_UNDEFINED)
    {
