...
//#define GLM_FORCE_SSE3 1 // TEST
#define GLM_FORCE_SSE42 1 // TEST

//#define GLM_FORCE_SIMD_AVX2 // TEST (prev)
...
#define PROJECT_POINT_OPTIM 1 //0
...
void
RayCaster::SetCameraAngles(RC_FLOAT angle0, RC_FLOAT angle1)
{
    mCamAngle0 = angle0;
    mCamAngle1 = angle1;
}
...
void
RayCaster::SetCameraFov(RC_FLOAT camFov)
{
    mCamFov = camFov;
...
void
RayCaster::ZoomChanged(RC_FLOAT zoomChange)
{
...
#if 0 // PROJECT_POINT_SIMD (old) (next version is better optimized)
void
RayCaster::ProjectPoints(vector<Point> *points, int width, int height)
{
    glm::mat4 model;
    ...
    for (int i = 0; i < points->size(); i++)
    {
        ...
        //
        RC_FLOAT x = ((float *)&v4)[0];
        RC_FLOAT y = ((float *)&v4)[1];
        RC_FLOAT z = ((float *)&v4)[2];
        RC_FLOAT w = ((float *)&v4)[3];
...
#else
        int x = (int)bl_round(normX*resolution[0]);
        int y = (int)bl_round(normY*resolution[1]);
#endif
...
RayCaster::DoRayCastQuadTree(vector<Point> *points, int width, int height)
{
    ...
    RC_FLOAT bbox[2][2] = { { (RC_FLOAT)0, (RC_FLOAT)0 },
                            { (RC_FLOAT)maxDim, (RC_FLOAT)maxDim } };
    //
...
void
RayCaster::SetColormapRange(RC_FLOAT range)
{
    if (mColorMap != NULL)
    {
        mColorMap->SetRange(range);
...
void
RayCaster::SetColormapContrast(RC_FLOAT contrast)
{
    if (mColorMap != NULL)
    {
        mColorMap->SetContrast(contrast);
...
bool
RayCaster::GetSelection(RC_FLOAT selection[4])
{
    for (int i = 0; i < 4; i++)
...
void
RayCaster::SetSelection(const RC_FLOAT selection[4])
{
    for (int i = 0; i < 4; i++)
...
#if 0 // OLD
bool
RayCaster::SelectionIntersectSlice2D(const RC_FLOAT selection2D[2][2],
                                     const RC_FLOAT projSlice2D[4][2])
{
...
RayCaster::GetCenterSlice()
{
    ...
    RC_FLOAT selection[4] = { mScreenSelection[0]*mViewWidth + 4.0, (1.0 - mScreenSelection[3])*mViewHeight + 4.0,
                            mScreenSelection[2]*mViewWidth + 1.0, (1.0 - mScreenSelection[1])*mViewHeight + 1.0 };
    
    RC_FLOAT selection2D[2][2] = { { selection[0], selection[1] },
    ...
    for (int i = 0; i < mSlices.size(); i++)
    {
        ...
        RC_FLOAT slice[4][3] = { { -0.5, -0.5, z },
                               { 0.5, -0.5, z },
                               { 0.5, 0.5, z },
                               { -0.5, 0.5, z } };
        
        // Project the slice
        RC_FLOAT projSlice[4][3];
        for (int k = 0; k < 4; k++)
        {
            ProjectPoint(projSlice[k], slice[k], mViewWidth, mViewHeight);
        }
        
        RC_FLOAT projSlice2D[4][2];
        for (int k = 0; k < 4; k++)
        {
...
void
RayCaster::TranslateVolumeSelection(RC_FLOAT trans[3])
{
    for (int i = 0; i < 8; i++)
    {
...
#if 0
void
RayCaster::ScreenToAlignedVolume(const RC_FLOAT screenSelectionNorm[4],
                                 RC_FLOAT alignedVolumeSelection[4])
{
    RC_FLOAT projFace[2][3];
    ProjectFace(projFace);
    
    RC_FLOAT screenSelection[4] = { screenSelectionNorm[0],
...
#if 0
void
RayCaster::AlignedVolumeToScreen(const RC_FLOAT alignedVolumeSelection[4],
                                 ...
                                 RC_FLOAT screenSelectionNorm[4])
{
    ...
    if ((mCamAngle0 < -EPS) && (mCamAngle1 > EPS))
        // Top face left
    {
        RC_FLOAT face[2][3] = { { mVolumeSelection[7/*4*/].mX, mVolumeSelection[7/*4*/].mY, mVolumeSelection[7/*4*/].mZ },
    ...
    if ((mCamAngle0 > EPS) && (mCamAngle1 > EPS))
        // Top face right
    {
        RC_FLOAT face[2][3] = { { mVolumeSelection[2/*1*/].mX, mVolumeSelection[2/*1*/].mY, mVolumeSelection[2/*1*/].mZ },
...
#if 0
void
RayCaster::ProjectFace(RC_FLOAT projFace[2][3])
{
  if ((std::fabs(mCamAngle0) < EPS) && (std::fabs(mCamAngle1) < EPS))
        // Front face
    {
        RC_FLOAT face[2][3] = { { 0.0, 0.0, 0.5 },
                              { 1.0, 1.0, 0.5 } };
...
RayCaster::ProjectFace(RC_FLOAT projFace[2][3])
{
  ...
  if ((std::fabs(mCamAngle0) < EPS) && (mCamAngle1 > EPS))
    ...
    if ((mCamAngle0 < -EPS) && (mCamAngle1 > EPS))
        // Top face left
    {
        RC_FLOAT face[2][3] = { { mVolumeSelection[7].mX, mVolumeSelection[7].mY, mVolumeSelection[7].mZ },
                              { mVolumeSelection[2].mX, mVolumeSelection[2].mY, mVolumeSelection[2].mZ } };
    ...
    if ((mCamAngle0 > EPS) && (mCamAngle1 > EPS))
        // Top face right
    {
        RC_FLOAT face[2][3] = { { mVolumeSelection[2].mX, mVolumeSelection[2].mY, mVolumeSelection[2].mZ },
...
RayCaster::ProjectFace(RC_FLOAT projFace[2][3])
{
  if ((std::fabs(mCamAngle0) < EPS) && (std::fabs(mCamAngle1) < EPS))
        // Front face
    {
        RC_FLOAT face[2][3] = { { 0.0, 0.0, mVolumeSelection[0].mZ },
  ...
  if ((mCamAngle0 > EPS) && (std::fabs(mCamAngle1) < EPS))
        // Left face
    {
        RC_FLOAT face[2][3] = { { mVolumeSelection[4].mX, 0.0, -0.5 },
  ...
  if ((mCamAngle0 < -EPS) && (std::fabs(mCamAngle1) < EPS))
        // Right face
    {
        RC_FLOAT face[2][3] = { { mVolumeSelection[1].mX, 0.0, 0.5 },
  ...
  if ((std::fabs(mCamAngle0) < EPS) && (mCamAngle1 > EPS))
        // Top face front
    {
        RC_FLOAT face[2][3] = { { -0.5, mVolumeSelection[3].mY, 0.5 },
...
void
RayCaster::ReorderScreenSelection(RC_FLOAT screenSelection[4])
{
    // Re-order the selection
...
void
RayCaster::SetQuality(RC_FLOAT quality)
{
    mQuality = quality;
...
void
RayCaster::SetAutoQuality(RC_FLOAT quality)
{
...
void
RayCaster::SetQualityT(RC_FLOAT qualityT)
{
    mQualityT = qualityT;
}

void
RayCaster::SetPointSize(RC_FLOAT size)
{
...
#if 0
void
RayCaster::SetAlphaScale(RC_FLOAT scale)
{
    mAlphaScale = scale;
}
#endif
...
void
RayCaster::SetAlphaCoeff(RC_FLOAT alphaCoeff)
{
    mAlphaCoeff = alphaCoeff;
}

void
RayCaster::SetThreshold(RC_FLOAT threshold)
{
    mThreshold = threshold;
}

void
RayCaster::SetThresholdCenter(RC_FLOAT thresholdCenter)
{
...
void
RayCaster::SetPlayBarPos(RC_FLOAT t)
{
    mPlayBarPos = t;
}
...
void
RayCaster::ProjectPoint(RC_FLOAT projP[3], const RC_FLOAT p[3], int width, int height)
{
    glm::mat4 model;
    glm::mat4 proj;
    CameraModelProjMat(width, height,
...
void
RayCaster::SetRenderAlgoParam(RC_FLOAT renderParam)
{
...
RayCaster::SelectPoints2D(vector<Point> *projPoints, int width, int height)
{
    ...
    RC_FLOAT selection[4] = { mScreenSelection[0]*width + 4.0, (1.0 - mScreenSelection[3])*height + 4.0,
                            ...
                            const deque<vector<RayCaster::Point> > &slicesQue)
{
    ...
    {
#else
    RC_FLOAT i1 = slicesQue.size() - 1 - (int)offset;
    while((int)i1 >= 0.0)
    {
#endif
        {
...
void
RayCaster::BlendDepthSorting(/*int*/ unsigned char rgba[4], vector<Point> &points)
{
    ...
    for (int k = 0; k < points.size(); k++)
    {
	  ...
	  weightAlphaCoeff = std::pow(mAlphaCoeff, (RC_FLOAT)1.0/p.mWeight);
        ...
        if (k == 0)
        {
            ...
            rgba0[0] = p.mR*selectCoeff;
            rgba0[1] = p.mG*selectCoeff;
            rgba0[2] = p.mB*selectCoeff;
            rgba0[3] = 255;
#else
            /*rgba0[0] += p.mR*colorCoeff;
            rgba0[1] += p.mG*colorCoeff;
            rgba0[2] += p.mB*colorCoeff; //*/
...
#else
            rgba0[3] += colorCoeff*alpha255; // ORIG
            
            // TEST 1: a bit better
            //rgba0[3] = colorCoeff*255;
            ...
            //rgba0[3] = colorCoeff*alpha255 + colorCoeff*alpha255;
        ...
        {
            /*rgba0[0] = (1.0 - colorCoeff)*rgba0[0] + colorCoeff*p.mR;
            rgba0[1] = (1.0 - colorCoeff)*rgba0[1] + colorCoeff*p.mG;
            rgba0[2] = (1.0 - colorCoeff)*rgba0[2] + colorCoeff*p.mB;*/
...
static RC_FLOAT
ComputeW(RC_FLOAT alpha, RC_FLOAT z)
{
  RC_FLOAT a = 3.0*1e3*std::pow((1.0 - z), 3.0);
    if (a < 1e-2)
        a = 1e-2;
...
RayCaster::BlendWBOIT(int rgba[4], vector<Point> &points0)
{
    ...
    for (int k = 0; k < points0.size(); k++)
    {
        ...
        normZ = BLUtils::LogScaleNorm2(normZ, 128.0);
        normZ = 1.0 - normZ;
        
        RC_FLOAT wi = normZ;
        //RC_FLOAT wi = ComputeW(mAlphaCoeff, p.mZ);
...
long
RayCaster::ComputeNumRayPoints(const vector<Point> &points, RC_FLOAT z)
{
    long numPoints = 0;
    for (int i = 0; i < points.size(); i++)
    {
