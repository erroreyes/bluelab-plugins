...
// on the middle slice
//
// Insert the middle slice in a special way
//
// NOTE: does'nt work well
#define FIX_OPTIM_PRESORT_ARTIFACT1 0

// With OPTIM_RES_SORT, there are some "depth" artifacts
// on the middle slice
//
// Tag the middle slice, and sort later on projected points
//
// NOTE: does'nt work well
#define FIX_OPTIM_PRESORT_ARTIFACT2 1
...
{    
    ...
    vector<vector<vector<Point2D> > > &grid = mTmpGrid;
    ...
    for (int i = 0; i < points.size(); i++)
    {
        ...
        RC_FLOAT normX = (projPoint.mXi - bbox[0][0])*xRatio;
        RC_FLOAT normY = (projPoint.mYi - bbox[0][1])*yRatio;
        ...
        int x = (int)round(normX);
        int y = (int)round(normY);
#endif
    ...
    {
        vector<vector<Point2D> > &line = grid[i];
        
        for (int j = 0; j < line.size(); j++)
        {
            vector<Point2D> &points0 = line[j];
...
{    
    ...
    for (int i = (int)sliceNumbers.size() - 1; i >= 0; i--)
    {
...
#if FIX_OPTIM_PRESORT_ARTIFACT1
        // Do not add the middle slice now, will add it later
        if (sliceNum == splitIdxZ)
            continue;
#endif
        ...
        for (int j = 0; j < vec.size(); j++)
        {
#if FIX_OPTIM_PRESORT_ARTIFACT2
          if (sliceNum == splitIdxZ)
...
#if FIX_OPTIM_PRESORT_ARTIFACT1
    // Add the middle slice, in a sorted way
    vector<RayCaster2::Point> &vec = slices[splitIdxZ];
    if (viewSignsChanged)
        SortVec(&vec, signX, signY, signZ);
    ...
    for (int j = 0; j < vec.size(); j++)
    {
        ...
        if (signY >= /**/0)
        {
            ...
            if (signX >= /**/0)
                insert_sorted(*points, val, Point::IsGreaterX);
            if (signX < 0)
                insert_sorted(*points, val, Point::IsSmallerX);
        }
        else if (signY < 0)
        {
            //if (signX >= /**/0)
            ...
            insert_sorted(*points, val, Point::IsGreaterY);
        }
    }
#endif
...
#define MAX_NUM_RAY_POINTS2 32
    ...
    RC_FLOAT ni = round(((RC_FLOAT)points->size())/MAX_NUM_RAY_POINTS2);
...
{
    WDL_TypedBuf<double> x;
    x.Resize(points.size());
    
    WDL_TypedBuf<double> y;
    ...
    Debug::DumpData(xFileName, x);
    
    char yFileName[255];
    sprintf(yFileName, "%s-y.txt", name);
    
    Debug::DumpData(yFileName, y);
}
                                       ...
                                       int *areaSign, RC_FLOAT *alignedCoord)
{
    ...
    while (indices[0] != indices[1])
    {
        ...
        else
        {
            ...
            RC_FLOAT diff01 = fabs(areas[0]) - fabs(areas[1]);
            RC_FLOAT diff02 = fabs(areas[2]) - fabs(areas[1]);
            if (fabs(diff01) < fabs(diff02))
            {
                indices[2] = indices[1];
            }
            else
        ...
        int minIndex = 0;
        RC_FLOAT minArea = fabs(areas[0]);
        if (fabs(areas[1]) < minArea)
        {
            minIndex = 1;
            minArea = fabs(areas[1]);
        }
        
        if (fabs(areas[2]) < minArea)
        {
            minIndex = 2;
        }
