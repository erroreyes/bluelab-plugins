//
//  USTPseudoStereoObj2.cpp
//  UST
//
//  Created by applematuer on 12/28/19.
//
//

#include <limits.h>

#include <randomsequence.h>

#include <FastRTConvolver2.h>
#include <USTProcess.h>
#include <FFTProcessObj16.h>
#include <DelayObj4.h>

#include <Utils.h>
#include <Debug.h>

#include "USTPseudoStereoObj2.h"

// Default value, will be modified in FastRTConvolver
// depending on host buffer size
#define BUFFER_SIZE 512

// For 44100Hz
#define IR_SIZE 2048

// Use delay for difference like in the paper ?
#define USE_DELAY 0

// Fix the gain loss after converting mono to stereo
// (see pan law principle)
// => Now the mono to stereo process is transparent in term of gain
//
// Gain in dB
#define GAIN_ADJUST_DB 6.0 //3.0


USTPseudoStereoObj2::USTPseudoStereoObj2(double sampleRate, double width)
{
    //
    FftProcessObj16::Init();
    
    mSampleRate = sampleRate;
    mWidth = width;
    
    SetIRSize(sampleRate);
    
    WDL_TypedBuf<double> ir;
    GenerateIR(&ir);
    
    mConvolver = new FastRTConvolver2(BUFFER_SIZE, ir);
 
#if USE_DELAY
    mDelayObj = new DelayObj4(1);
    UpdateDelay();
#endif
}

USTPseudoStereoObj2::~USTPseudoStereoObj2()
{
    delete mConvolver;
    
#if USE_DELAY
    delete mDelayObj;
#endif
}

void
USTPseudoStereoObj2::Reset(double sampleRate, int blockSize)
{
    mSampleRate = sampleRate;

    mConvolver->Reset(blockSize);
    
    SetIRSize(sampleRate);
    
    WDL_TypedBuf<double> ir;
    GenerateIR(&ir);
    mConvolver->SetIR(ir);
 
#if USE_DELAY
    UpdateDelay();
#endif
}

int
USTPseudoStereoObj2::GetLatency()
{
    int latency = mConvolver->GetLatency();
    
    return latency;
}


void
USTPseudoStereoObj2::SetWidth(double width)
{
    mWidth = width;
}

void
USTPseudoStereoObj2::ProcessSamples(const WDL_TypedBuf<double> &sampsIn,
                                 WDL_TypedBuf<double> *sampsOutL,
                                 WDL_TypedBuf<double> *sampsOutR)
{
    sampsOutL->Resize(sampsIn.GetSize());
    mConvolver->Process(sampsIn, sampsOutL);
    
    // TEST: simple substraction without delay
    //*sampsOutR = sampsIn;
    //Utils::SubstractValues(sampsOutR, *sampsOutL);
    
    // Substract with delay (see paper, "group delay")
    sampsOutR->Resize(sampsIn.GetSize());
    for (int i = 0; i < sampsIn.GetSize(); i++)
    {
        double samp = sampsIn.Get()[i];
        
#if !USE_DELAY
        double sampD = samp;
#else
        double sampD = mDelayObj->ProcessSample(samp);
#endif
        double sampL = sampsOutL->Get()[i];
        double sampR = sampD - sampL;
        
        sampsOutR->Get()[i] = sampR;
    }
    
    // Adjust the gain so the processing is transparent
    // when doing nothing
    // (otherwise the sound would be lower)
    AdjustGain(sampsOutL);
    AdjustGain(sampsOutR);
}

void
USTPseudoStereoObj2::ProcessSamples(vector<WDL_TypedBuf<double> > *samplesVec)
{
    if (samplesVec->empty())
        return;
    
    if (samplesVec->size() != 2)
        return;
    
    USTProcess::StereoToMono(samplesVec);
    
    WDL_TypedBuf<double> mono = (*samplesVec)[0];
    ProcessSamples(mono, &(*samplesVec)[0], &(*samplesVec)[1]);
    
    // No need, if we want to adjut the default width,
    // change the DEFAULT_WIDTH macro in this class
#if 0
    // Adjust the width by default
#define WIDTH_ADJUST 1.0
    vector<WDL_TypedBuf<double> * > samples;
    samples.push_back(&(*samplesVec)[0]);
    samples.push_back(&(*samplesVec)[1]);
    
    USTProcess::StereoWiden(&samples, WIDTH_ADJUST);
#endif
}

void
USTPseudoStereoObj2::GenerateIR(WDL_TypedBuf<double> *ir)
{
    ir->Resize(mIRSize);
    
    // Use a random number class
    // So we always have the same seed, without changing the system seed
    // (so the result sound will be always the same)
    unsigned int seedBase = 2345677898907;
    RandomSequenceOfUnique rnd(seedBase, seedBase + 1);
    
    // Generate the random number vector
    WDL_TypedBuf<double> R;
    R.Resize(ir->GetSize());
    for (int i = 0; i < R.GetSize(); i++)
    {
        unsigned int r = rnd.next();
        
        double rd = ((double)r)/UINT_MAX;
        
        rd = (rd - 0.5)*2.0;
        
        R.Get()[i] = rd;
    }
    
    // Compute Hl
    WDL_TypedBuf<double> Hl;
    Hl.Resize(ir->GetSize());
    for (int i = 0; i < Hl.GetSize(); i++)
    {
        double wr = mWidth*mWidth*R.Get()[i];
        double at = atan(wr);
        
        double h = 1.0/2.0 + (1.0/M_PI)*at;
        
        Hl.Get()[i] = h;
    }
    
    // Compute Fft-1(Hl)
    WDL_TypedBuf<double> hl;
    FftProcessObj16::ComputeInverseFft(Hl, &hl, true);
    
    *ir = hl;
    
    // BAD: shifts the pan to the left
    //NormalizeIR(ir);
}

void
USTPseudoStereoObj2::SetIRSize(double sampleRate)
{
    mIRSize = IR_SIZE*(sampleRate/44100.0);
    mIRSize = Utils::NextPowerOfTwo(mIRSize);
}

void
USTPseudoStereoObj2::UpdateDelay()
{
#if USE_DELAY
    double delay = (mIRSize - 1)*0.5;
    
    mDelayObj->SetDelay(delay);
#endif
}

void
USTPseudoStereoObj2::NormalizeIR(WDL_TypedBuf<double> *ir)
{
    double sum = Utils::ComputeSum(*ir);
    
#define EPS 1e-10
    if (fabs(sum) > EPS)
    {
        Utils::MultValues(ir, 1.0/sum);
    }
}

void
USTPseudoStereoObj2::AdjustGain(WDL_TypedBuf<double> *samples)
{
    double gain = DBToAmp(GAIN_ADJUST_DB);
    
    Utils::MultValues(samples, gain);
}
