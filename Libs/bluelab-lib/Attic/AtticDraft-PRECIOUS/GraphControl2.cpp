	...
	mVAxis(NULL)
{
    ...
    // With a coeff of 4, we will redraw 4 times when one a single point will arrive
    // For 25 fps, with a coeff of 2, we will get 1 buffers of 1024 samples at 44100 Hz
    // TODO: test at higher framerates
    mSmoothStreamCoeff = 8;
}
...
#if USE_GRAPH_ADJUSTED_SCROLLING
void
GraphControl2::UpdateScrolling()
{
    if (mPointsCounter++ % mSmoothStreamCoeff == 0)
        mNumPointsPushed++;
...
void
GraphControl2::LockScrolling()
{
    mScrollMutex.Enter();
}

void
GraphControl2::UnlockScrolling()
{
    mScrollMutex.Leave();
}

void
GraphControl2::SetScrollParams(int sampleRate, int numSamplesPoint)
{
    mSampleRate = sampleRate;
    mNumSamplesPoint = numSamplesPoint;
}
...
GraphControl2::PushCurveValue(int curveNum, double val)
{
    ...
    // WARNING: the following code looks very smoggy...
#if USE_GRAPH_ADJUSTED_SCROLLING
    mCurves[curveNum]->mTmpPushValue += val;
    
    if (mCurves[curveNum]->mPushCounter++ % mSmoothStreamCoeff == 0)
    {
        mCurves[curveNum]->PushValue(mCurves[curveNum]->mTmpPushValue/mSmoothStreamCoeff);
    
        mCurves[curveNum]->mTmpPushValue = 0.0;
...
#if 0
            t = Utils::NormalizedYTodB2(t, minYdB, maxYdB);
#endif
...
#if USE_GRAPH_ADJUSTED_SCROLLING
void
GraphControl2::AdjustScrolling()
{
    // Scroll adjustment
    if (mSampleRate > 0)
    {
        if (mFirstTime && mNumPointsPushed > 0)
        {
            mFirstTime = false;
        ...
        // Num points in theory, depending on the elapsed time
        double points = MillisToPoints(elapsed, mSampleRate, mNumSamplesPoint*mSmoothStreamCoeff);
        
        // Translate automatically depending on the time
        int width = this->mRECT.W();
        mScrollTransX -= (points/mNumCurveValues)*width;
        
        // We have points, translate back
        if (mNumPointsPushed > 0)
        {
            mScrollTransX += mNumPointsPushed*width/((double)mNumCurveValues);
            
            mNumPointsPushed = 0;
        }
        
        // If we dont receive points, then stop scrolling
        if (mNumPointsPushed > 0)
            mTimer2.Reset();
        
        unsigned long long elapsed2 = mTimer2.Get();
        //if (elapsed2 > 100*mSmoothStreamCoeff)
        if (elapsed2 > 20)
        // 100 ms = 2 frames at 25fps
        {
            // Will make a small jump when restarting
            mScrollTransX = 0;
    ...
    nvgTranslate(mVg, mScrollTransX, 0.0);
    
    // Scale a little, to avoid empty area on the right
    nvgScale(mVg, 1.02, 1.0);
}
#endif
