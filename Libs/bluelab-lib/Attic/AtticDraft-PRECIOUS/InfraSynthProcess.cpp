...
InfraSynthProcess::InfraSynthProcess(BL_FLOAT sampleRate)
{
    ...
    mLowFilter = new IIRFilterLow12dB();
    mLowFilter->Init(LOW_FREQ_CUTOFF, sampleRate);
#endif
    
#if FIX_HIGH_FREQS_SUB
    mSubLowFilter = new IIRFilterLow12dB();
                              ...
                              BL_FLOAT *phantomSamp, BL_FLOAT *subSamp)
{
    ...
    BL_FLOAT phantomMix = Utils::ApplyParamShape(mPhantomMix, 0.5);
#else
...
#if !OPTIM_APPLY_PARAM_SHAPE
    BL_FLOAT subMix = Utils::ApplyParamShape(mSubMix, 0.5);
#else
...
InfraSynthProcess::AmplifyOriginSample(BL_FLOAT sample)
{
    ...
    BL_FLOAT dbCoeff = mPhantomMix*DBToAmp(INITIAL_INCREASE_DB);
                                           ...
                                           bool fixedPhantomFreq)
{
    ...
    BL_FLOAT phantomInitAmp = DBToAmp(-PHANTOM_INIT_DB);
    amp *= phantomInitAmp;
#endif
    ...
    while(partialNum < PHANTOM_NUM_PARTIALS)
    {
        ...
        if (mEnableStartSync)
        {
            //p0.mPhase = ComputeSyncedPhase(p0.mPhase, partial.mFreq, p0.mFreq);
            p0.mPhase = 0.5*M_PI;
        }
        
        if (phantNum == phantToKeep) // DEBUG TEST
        {
...
#else
        BL_FLOAT decreaseAmp = DBToAmp(-numOctave*DECREASE_DB);
        amp *= decreaseAmp;
#endif
...
#else
    BL_FLOAT decreaseAmp = DBToAmp(DECREASE_DB);
    BL_FLOAT amp = partial.mAmp*decreaseAmp;

    // Add more db, to be like in Infra plug
    BL_FLOAT subOffsetAmp = DBToAmp(SUB_FREQ_DB_OFFSET);
...
#if 0
BL_FLOAT
InfraSynthProcess::ComputeSyncedPhase(BL_FLOAT currentPhase,
                                      BL_FLOAT refFreq,
                                      BL_FLOAT currentFreq)
{
    if (currentFreq > refFreq)
        currentPhase += 0.5*M_PI*refFreq/currentFreq - 0.5*M_PI; // Forks for 1st partial (ratio: 4/5)
    else
    {
        // Works for octave frequencies
        while(refFreq > currentFreq)
        {
            currentPhase += 0.5*M_PI*currentFreq/refFreq;
