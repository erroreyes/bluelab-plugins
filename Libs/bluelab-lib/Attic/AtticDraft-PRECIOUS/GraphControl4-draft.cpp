	...
	mVAxis(NULL)
{
    ...
    mPixels = (unsigned char *)malloc(width*height*4*sizeof(unsigned char));
...
GraphControl4::Draw(IGraphics *pGraphics)
{
    ...
    BitBlt(graphicsDC, r->L, r->T, r->W(), r->H(), fbDC, 0, 0, LICE_EXT_BLIT_ACCEL /*SRCCOPY*/);
#endif
...
// Unused
void
GraphControl4::SwapChannels(unsigned int *data, int numPixels)
{
...
GraphControl4::DrawGL()
{
    ...
    if (mLiceFb == NULL)
    {
        //mLiceFb = new LICE_SysBitmap(0,0);
    ...
    // DEBUG
    LICE_pixel color = LICE_RGBA(0x52, 0x52, 0xFF, 0xFF);
    LICE_Clear(mLiceFb, color);
