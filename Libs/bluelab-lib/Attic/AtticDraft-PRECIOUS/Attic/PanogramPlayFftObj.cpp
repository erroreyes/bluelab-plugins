...
// Align better the sound played with the grpahics
#define FIX_SHIFT_LINE_COUNT 1
...
#define FIX_LAST_LINES_BAD_SOUND 0 //1
                                     ...
                                     const WDL_TypedBuf<WDL_FFT_COMPLEX> *scBuffer)
{
    ...
    if (mMode == PLAY)
    {
        ...
        if (!mSelectionPlayFinished)
        {
            ...
            inside = ShiftLineCount(&lineCount);
#endif
            ...
            else
            {
                ...
                Utils::FillAllZero(&magns);
                Utils::FillAllZero(&phases);
            }
        }
    ...
    if (mMode == RECORD)
    {
        ...
        Utils::TakeHalf(&ioBuffer0);
        
        WDL_TypedBuf<double> magns;
        WDL_TypedBuf<double> phases;
        Utils::ComplexToMagnPhase(&magns, &phases, ioBuffer0);
...
void
PanogramPlayFftObj::Reset(int bufferSize, int oversampling, int freqRes, double sampleRate)
{
    ProcessObj::Reset(bufferSize, oversampling, freqRes, sampleRate);
...
void
PanogramPlayFftObj::SetNormSelection(double x0, double y0, double x1, double y1)
{
    // Swap
    double newY0 = 1.0 - y1;
    double newY1 = 1.0 - y0;
...
void
PanogramPlayFftObj::GetNormSelection(double selection[4])
{
...
void
PanogramPlayFftObj::RewindToNormValue(double value)
{
...
{
    double res = ((double)mLineCount)/mNumCols;
...
{
    double res = ((double)(mLineCount - mDataSelection[0]))/
...
void
PanogramPlayFftObj::GetDataLine(const deque<WDL_TypedBuf<double> > &inData,
                                WDL_TypedBuf<double> *data,
                                int lineCount)
{
    double x0 = mDataSelection[0];
    double x1 = mDataSelection[2];
...
void
PanogramPlayFftObj::GetDataLineMask(const deque<WDL_TypedBuf<double> > &inData,
                                    WDL_TypedBuf<double> *data,
                                    int lineCount)
{
    double x0 = mDataSelection[0];
...
bool
PanogramPlayFftObj::ShiftLineCount(int *lineCount)
{
    // Shift a little, to be exactly at the middle of the selection
    *lineCount += mOverlapping*2;
    if (*lineCount >= mNumCols)
    {
        *lineCount = mNumCols - 1;
