...
void
FftProcessObj14::Reset(int overlapping, int freqRes, double sampleRate)
{
    mOverlapping = overlapping;
    mFreqRes = freqRes;
    mSampleRate = sampleRate;
...
void
FftProcessObj14::SamplesToHalfMagnPhases(const WDL_TypedBuf<double> &inSamples,
                                         WDL_TypedBuf<double> *outFftMagns,
                                         WDL_TypedBuf<double> *outFftPhases)
{
    int numSamples = inSamples.GetSize();
...
void
FftProcessObj14::MagnsToCepstrum(const WDL_TypedBuf<double> &halfMagns,
...
void
FftProcessObj14::HalfMagnPhasesToSamples(const WDL_TypedBuf<double> &fftMagns,
                                         const  WDL_TypedBuf<double> &fftPhases,
                                         WDL_TypedBuf<double> *outSamples)
{
...
void
FftProcessObj14::AddSamples(const vector<WDL_TypedBuf<double> > &inputs,
...
bool
FftProcessObj14::GetResults(vector<WDL_TypedBuf<double> > *outputs,
                           int numRequested)
{
...
bool
FftProcessObj14::Process(const vector<WDL_TypedBuf<double> > &inputs,
                         const vector<WDL_TypedBuf<double> > &scInputs,
...
void
FftProcessObj14::GetAllSamples(vector<WDL_TypedBuf<double> * > *samples)
{
    for (int i = 0; i < mChannels.size(); i++)
    {
        ProcessObjChannel *chan = mChannels[i];
...
void
FftProcessObj14::GetAllResultSamples(vector<WDL_TypedBuf<double> * > *samples)
{
    for (int i = 0; i < mChannels.size(); i++)
    {
        ProcessObjChannel *chan = mChannels[i];
