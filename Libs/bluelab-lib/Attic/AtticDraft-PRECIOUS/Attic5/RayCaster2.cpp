...
{
// TODO:
// - fix the junction line, which is incorrect (empty)
// - re-activate and check well anti-jitter (limit number of slices to check)
// - presort x and y : check, and check when to do this !
// (check other TODOs)
// NOTE: prev perfs with std sort: 65%. Target perfs: 59%
    ...
    RC_FLOAT alignedZ;
    //RC_FLOAT fixedZ = 0.5;
    int splitIdxZ = ComputeMostAlignedSlice(mViewWidth, mViewHeight, AXIS_Z,
    ...
    //vector<int> dbgSliceIds;
    //vector<double> dbgSliceIds1;
    //vector<double> dbgSliceIds2;
    ...
    //if (signZ > 0)
    {
        splitIdxZ++;
        //splitIdxZ += step; // TEST
        if (splitIdxZ > (int)slices.size() - 1)
            splitIdxZ = (int)slices.size() - 1;
    }
...
#if 1 // Draw the background
    RC_FLOAT i1 = splitIdxZ - /*1*/step + /*(int)*/offset;
    //RC_FLOAT i1 = splitIdxZ - step - (int)offset;
    while((int)i1 >= 0.0)
    {
        /*const*/ vector<RayCaster2::Point> &vec = slices[(int)i1];
        
        //dbgSliceIds.push_back((int)i1);
        //dbgSliceIds1.push_back(/*(int)*/i1);
...
#if 0 //1 // Draw the background (reversed)
    RC_FLOAT i1 = (int)offset;
    while((int)i1 </*=*/ splitIdx)
    {
        const vector<RayCaster2::Point> &vec = slices[(int)i1];
...
#if 1 // Draw the foreground
    RC_FLOAT i2 = splitIdxZ /*+ 1*/ + /*(int)*/offset;
    while((int)i2 < slices.size())
    {

        /*const */vector<RayCaster2::Point> &vec = slices[(int)i2];
        ...
        //dbgSliceIds.push_back((int)i2);
        //dbgSliceIds2.push_back(/*(int)*/i2);
...
#if 0 //1 // Draw the foreground (reversed)
    RC_FLOAT i2 = slices.size() - 1 - (int)offset;
    while((int)i2 >/*=*/ splitIdx)
    {
    ...
    /* sort(dbgSliceIds.begin(), dbgSliceIds.end());
    WDL_TypedBuf<double> dbgSliceIdsWd;
    dbgSliceIdsWd.Resize(dbgSliceIds.size());
    for (int i = 0; i < dbgSliceIdsWd.GetSize(); i++)
        dbgSliceIdsWd.Get()[i] = dbgSliceIds[i];
    Debug::DumpData("slices.txt", dbgSliceIdsWd);
    
    WDL_TypedBuf<double> dbgSliceIdsWd1;
    dbgSliceIdsWd1.Resize(dbgSliceIds1.size());
    for (int i = 0; i < dbgSliceIdsWd1.GetSize(); i++)
        dbgSliceIdsWd1.Get()[i] = dbgSliceIds1[i];
    Debug::DumpData("slices1.txt", dbgSliceIdsWd1);
    
    WDL_TypedBuf<double> dbgSliceIdsWd2;
    dbgSliceIdsWd2.Resize(dbgSliceIds2.size());
    for (int i = 0; i < dbgSliceIdsWd2.GetSize(); i++)
        dbgSliceIdsWd2.Get()[i] = dbgSliceIds2[i];
    Debug::DumpData("slices2.txt", dbgSliceIdsWd2);
    
    Debug::DumpValue("idx.txt", splitIdx); */
}
...
RayCaster2::DBG_DrawSlice(NVGcontext *vg, int width, int height,
                          RC_FLOAT slice[4][3], bool highlight)
{
    nvgSave(vg);
...
{
    //
    int alignIdxZ = ComputeMostAlignedSlice(width, height, AXIS_Z);
...
RayCaster2::DBG_DrawSliceX(NVGcontext *vg, int width, int height, RC_FLOAT x)
{
    ...
    DBG_DrawSlice(vg, width, height, slice);
}
