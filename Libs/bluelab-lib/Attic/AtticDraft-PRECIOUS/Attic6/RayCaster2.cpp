...
// NOTE: prev perfs with std sort: 65%. Target perfs: 59%
    ...
    for (int i = 0; i < sliceNumbers.size(); i++)
    {
        ...
        RC_FLOAT dist = abs(sliceNum - splitIdxZ);
        //RC_FLOAT dist = sliceNum - splitIdxZ;
        if (dist < minDist)
        {
            minDist = dist;
...
#if 0 //1 // Draw the background
    RC_FLOAT i1 = splitIdxZ - /*1*/step + /*(int)*/offset;
    while((int)i1 >= offset/*0.0*/) // GOOD !
    {
        vector<RayCaster2::Point> &vec = slices[(int)i1];
        ...
        DBG_SliceIds.push_back((int)i1);
        DBG_SliceIds1.push_back(i1);
        
        i1 -= step;
    }
#endif
    
#if 0 //1 // Draw the foreground
    RC_FLOAT i2 = splitIdxZ /*+ 1*/ + /*(int)*/offset;
    while((int)i2 < slices.size())
    {

        vector<RayCaster2::Point> &vec = slices[(int)i2];
        ...
        DBG_SliceIds.push_back((int)i2);
        DBG_SliceIds2.push_back(i2);
