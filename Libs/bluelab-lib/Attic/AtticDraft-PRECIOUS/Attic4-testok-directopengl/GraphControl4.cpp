...
GraphControl4::Draw(IGraphics *pGraphics)
{
...
#if 0
    LICE_IBitmap *graphicsBmp = pGraphics->GetDrawBitmap();
    ...
    // TODO: draw bitmap normally, then it should be accelerated !
    LICE_Blit(graphicsBmp, mLiceFb, 0, 0, &srcRect, 1.0, LICE_EXT_BLIT_ACCEL);
#endif
...
#if 1   // Bug with direct rendering
...
// Take care of the following macro in Lice ! : DISABLE_LICE_EXTENSIONS
    ...
    glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, fbo);  // bind this FBO so it is the rendering target
    int valid = (glCheckFramebufferStatusEXT(GL_FRAMEBUFFER_EXT) == GL_FRAMEBUFFER_COMPLETE_EXT);
    if (valid)
    {
    ...
    //int status;
    //glGetIntegerv(GL_STENCIL_BITS, &status);
