...
RayCaster2::AddSlice(const vector<Point> &points, bool skipDisplay)
{
    ...
    //stable_sort(points0.begin(), points0.end(), Point::IsSmallerX);
    
    //sort(points0.begin(), points0.end(), Point::IsSmallerX);
    //stable_sort(points0.begin(), points0.end(), Point::IsSmallerY);
    
    //sort(points0.begin(), points0.end(), Point::IsGreaterY);
    //stable_sort(points0.begin(), points0.end(), Point::IsGreaterX);
    
    //sort(points0.begin(), points0.end(), Point::IsGreaterX);
    //stable_sort(points0.begin(), points0.end(), Point::IsGreaterY);
    
#if 1 // Should be here!
    PreSortPoints(&points0);
#endif
...
#if DEBUG_DICHO
    // DEBUG
    static int maxNumPoints0 = 0;
    static int minNumPoints0 = 1000000;
    static int numPoints0 = 0;
    static int sumNumPoints0 = 0;
    
    static int maxNumPoints1 = 0;
    static int minNumPoints1 = 1000000;
    static int numPoints1 = 0;
    static int sumNumPoints1 = 0;
    //
#endif
...
#define EPS_ALPHA 1
...
#if DEBUG_DICHO
            // DEBUG
            if (points0.size() > maxNumPoints0)
                maxNumPoints0 = points0.size();
            if (points0.size() < minNumPoints0)
                minNumPoints0 = points0.size();
            numPoints0++;
            sumNumPoints0 += points0.size();
#endif
...
#if DEBUG_DICHO
            // DEBUG
            if (points0.size() > maxNumPoints1)
                maxNumPoints1 = points0.size();
            if (points0.size() < minNumPoints1)
                minNumPoints1 = points0.size();
            numPoints1++;
            sumNumPoints1 += points0.size();
#endif
    ...
    // DEBUG
    if (numPoints0 > 0)
        fprintf(stderr, "0: (%d -> %d) avg: %d\n",
                minNumPoints0, maxNumPoints0, sumNumPoints0/numPoints0);
    
    
    if (numPoints1 > 0)
        fprintf(stderr, "1: (%d -> %d) avg: %d\n",
                minNumPoints1, maxNumPoints1, sumNumPoints1/numPoints1);
#endif
}
...
void
RayCaster2::SlicesDequeToVec(vector<RayCaster2::Point> *points,
                            const deque<vector<RayCaster2::Point> > &slicesQue)
{
    ...
    long elementId = 0;
#if (!REVERT_DISPLAY_ORDER && !OPTIM_PRE_SORT)
    RC_FLOAT i1 = offset;
...
#endif
        {
            const vector<RayCaster2::Point> &vec = slicesQue[(int)i1];
            //vector<RayCaster2::Point> &vec = ((deque<vector<RayCaster2::Point> > &)slicesQue)[(int)i1];
            //PreSortPoints(&vec);
...
#if OPTIM_PRE_SORT // TEST (should not be here, for perfs)
    //PreSortPoints(points);
#endif
}
#else
void
                             ...
                             const deque<vector<RayCaster2::Point> > &slicesQue)
{
    ...
    while((int)i1 >= 0.0)
    {
        ...
        // Warning: here, we insert !
        InsertSorted(points, vec);
...
RayCaster2::BlendDepthSorting(unsigned char rgba[4], vector<Point> &points)
{
    ...
    // Avoid reverse() => gain 1 or 2% perfs
...
#if 0 // TEST
void
RayCaster2::SortFTB(vector<Point> *points)
{
//#define SORT_LIMIT 30 // >
//#define SORT_LIMIT 100 // 65%
#define SORT_LIMIT 150 // 63%
//#define SORT_LIMIT 200 // 62/63%
    
    if (points->size() < SORT_LIMIT)
    {
        sort(points->begin(), points->end(), Point::IsSmallerZ);
    ...
    pointers.resize(points->size());
    for (int i = 0; i < points->size(); i++)
    {
        pointers[i] = &(*points)[i];
    }
                    
    sort(pointers.begin(), pointers.end(), Point::IsSmallerZP);
...
#define SORT_LIMIT 150
    ...
    for (int i = 0; i < points->size(); i++)
    {
        ...
        insert_sorted(result, p, Point::IsSmallerZ);
    }
...
#define ANGLE1_LIMIT0 20.0 // fail bottom
//#define ANGLE1_LIMIT 0.0 // fail top
    
#define ANGLE1_LIMIT1 70.0
    
void
RayCaster2::PreSortPoints(vector<Point> *points)
{
    // No need to sort x (because the camera doesn't go "behind")
    //sort(points->begin(), points->end(), Point::IsSmallerX);
    
#if 0
    if (mCamAngle1 > ANGLE1_LIMIT0)
    //if ((mCamAngle1 > ANGLE1_LIMIT0) && (mCamAngle1 < ANGLE1_LIMIT1))
    /*stable_*/sort(points->begin(), points->end(), Point::IsGreaterY);
    else
    /*stable_*/sort(points->begin(), points->end(), Point::IsSmallerY);
#endif
    
    // TEST
    stable_sort(points->begin(), points->end(), Point::IsSmallerY);
    stable_sort(points->begin(), points->end(), Point::IsSmallerX);
    
    //fprintf(stderr, "angle0: %g angle1: %g\n", mCamAngle0, mCamAngle1);
    ...
    for (int i = 0; i < points->size(); i++)
    {
        ...
        p.mWeight = ((RC_FLOAT)i)/(points->size());
        //p.mWeight = p.mY;
        //p.mY = 0.0;
    }
#endif
...
void
RayCaster2::InsertSorted(vector<Point> *points, const vector<Point> &newPoints)
{
#if 0 // Very very inefficient
    for (int i = 0; i < newPoints.size(); i++)
    {
        const Point &p = newPoints[i];
        
        if (mCamAngle1 > ANGLE1_LIMIT0)
            insert_sorted(*points, p, Point::IsGreaterZ);
        else
            insert_sorted(*points, p, Point::IsSmallerZ);
    }
#endif
    
#if 0 // Inefficent (and buggy)
    vector<Point> result;
    result.resize(points->size() + newPoints.size());
    
    if (mCamAngle1 > ANGLE1_LIMIT0)
        merge(points->begin(), points->end(),
              newPoints.begin(), newPoints.end(),
              result.begin(), Point::IsGreaterZ);
    else
        merge(points->begin(), points->end(),
              newPoints.begin(), newPoints.end(),
              result.begin(), Point::IsSmallerZ);
    
    *points = result;
#endif
    
#if 0 // Very inefficient
    points->insert(points->end(), newPoints.begin(), newPoints.end());
    
    if (mCamAngle1 > ANGLE1_LIMIT0)
        inplace_merge(points->begin(), points->begin() + newPoints.size(),
                      points->end(), Point::IsGreaterZ);
    else
        inplace_merge(points->begin(), points->begin() + newPoints.size(),
                      points->end(), Point::IsSmallerZ);
#endif
