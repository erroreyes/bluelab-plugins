...
FftProcessObj14::ProcessSamples()
{
    ...
    while(chan0->HasSamplesToProcess())
    {
        ...
        while(true)
        {
            ...
            for (int i = mChannels.GetSize() - 1; i >= 0 ; i--)
            {
                ProcessObjChannel *chan = mChannels.Get()[i];
...
FftProcessObj14::SetupSideChain()
{
    ...
    int numInputs = mChannels.GetSize() - mNumScInputs;
    for (int i = 0; i < numInputs; i++)
    {
        ProcessObjChannel *chan = mChannels.Get()[i];
        
        if (i == mNumScInputs)
        // Exactly one side chain corresponding to this channel
        {
            ProcessObjChannel *sc = mChannels.Get()[i + numInputs];
        ...
        // Set side chain 0
        ProcessObjChannel *sc = mChannels.Get()[numInputs];
        chan->SetSideChain(sc);
    }
}
