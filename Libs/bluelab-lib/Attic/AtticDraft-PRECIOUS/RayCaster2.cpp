...
void
RayCaster2::SlicesDequeToVec(vector<RayCaster2::Point> *res,
...
void
RayCaster2::BlendDepthSorting(/*int*/ unsigned char rgba[4], vector<Point> &points)
{
    ...
    for (int i = 0; i < points.size(); i++)
    {
        ...
        for (int j = (int)(x - size[0]/2); j < (int)(x + size[0]/2); j++)
            for (int k = (int)(y - size[1]/2); k < (int)(y + size[1]/2); k++)
            {
                ...
                //*((int *)&mImageData.Get()[(j + k*width)*4]) = *((int *)&rgba);
...
void
RayCaster2::DiscardFartestRayPoints(vector<Point> *points)
{
