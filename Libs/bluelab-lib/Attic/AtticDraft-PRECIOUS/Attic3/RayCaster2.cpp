...
#if DBG_DRAW_SLICES
    DBG_DrawSlices(vg, width, height);
#endif
...
RayCaster2::AddSlice(const vector<Point> &points, bool skipDisplay)
{
    ...
    // TODO: do this when extracting vec from slices ?
#if OPTIM_PRE_SORT
...
//#if DBG_DRAW_SLICES
//        if (!vec.empty())
//            mDBG_SlicesZ.push_back(vec[0].mZ);
//#endif
    ...
    // DEBUG OPTIM_PRE_SORT = sort the whole points, just for testing
...
void
RayCaster2::DBG_DrawSlices(NVGcontext *vg, int width, int height)
{
    int alignIdx = ComputeAlignedSliceIdx(width, height);
...
// See: https://www.tutorialspoint.com/maximum-area-of-quadrilateral-in-cplusplus
static RC_FLOAT
GetMaxQuadArea2(RC_FLOAT a, RC_FLOAT b, RC_FLOAT c, RC_FLOAT d)
{
    RC_FLOAT s = (a + b + c + d)/2.0;
    RC_FLOAT area2 = (s - a) * (s - b) * (s - c) * (s - d);
    //area = sqrt(area2)
...
int
RayCaster2::ComputeAlignedSliceIdx(int width, int height)
{
    int index = -1;
    RC_FLOAT minDim = INF;
    ...
    for (int i = 0; i < mSlices.size(); i++)
    {
        ...
        //
        RC_FLOAT dx[4] = { projectedSlice[1][0] - projectedSlice[0][0],
                           projectedSlice[2][0] - projectedSlice[1][0],
                           projectedSlice[3][0] - projectedSlice[2][0],
                           projectedSlice[0][0] - projectedSlice[3][0] };
        
        RC_FLOAT dy[4] = { projectedSlice[1][1] - projectedSlice[0][1],
                           projectedSlice[2][1] - projectedSlice[1][1],
                           projectedSlice[3][1] - projectedSlice[2][1],
                           projectedSlice[0][1] - projectedSlice[3][1] };
        
        // Find the minimum dimension
        for (int k = 0; k < 4; k++)
        {
            if (fabs(dx[k]) + fabs(dx[(k + 2)%4]) < minDim)
            {
                minDim = fabs(dx[k]) + fabs(dx[(k + 2)%4]);
                
                index = i;
            }
            
            if (fabs(dy[k]) + fabs(dy[(k + 2)%4]) < minDim)
            {
                minDim = fabs(dy[k]) + fabs(dy[(k + 2)%4]);
        ...
        RC_FLOAT bbox[2][2] = { { INF, INF }, { -INF, -INF } };
        for (int k = 0; k < 4; k++)
        {
            RC_FLOAT x = projectedSlice[k][0];
            if (x < bbox[0][0])
                bbox[0][0] = x;
            if (x > bbox[1][0])
                bbox[1][0] = x;
            
            RC_FLOAT y = projectedSlice[k][1];
            if (y < bbox[0][1])
                bbox[0][1] = y;
            if (y > bbox[1][1])
                bbox[1][1] = y;
        ...
        // Bbox size
        RC_FLOAT boxWidth = bbox[1][0] - bbox[0][0];
        RC_FLOAT boxHeight = bbox[1][1] - bbox[0][1];
        ...
        for (int k = 0; k < 4; k++)
        {
            RC_FLOAT x = projectedSlice[(k + 1) % 4][0] - projectedSlice[k][0];
            RC_FLOAT y = projectedSlice[(k + 1) % 4][1] - projectedSlice[k][1];
            
            sides[k] = sqrtf(x*x + y*y);
        }
        
        RC_FLOAT maxArea2 = GetMaxQuadArea2(sides[0], sides[1], sides[2], sides[3]);
