...
#define OPTIM_PRE_SORT_SORT_CAM 0 //1

#define DBG_DRAW_SLICES 1 //0 //1
...
RayCaster2::AddSlice(const vector<Point> &points, bool skipDisplay)
{
    ...
    // TODO: find a way to even compute colormap,
    // while avoiding a struct size > 32
#if !(OPTIM_PRE_SORT && OPTIM_PRE_SORT_SORT_CAM)
...
//#define EPS 1e-15
    ...
    //int areaSignZ;
    int splitIdxZ = ComputeMostAlignedSliceIdx(mViewWidth, mViewHeight, AXIS_Z); //, &areaSignZ);
#else
    int splitIdxZ = ComputeMostAlignedSliceIdxDicho(mViewWidth, mViewHeight, AXIS_Z);
#endif
    
    //fprintf(stderr, "areaSignZ: %d\n", areaSignZ);
    //if (areaSignZ < 0)
    //   splitIdxZ++;
    //if (areaSignZ > 0)
    //    splitIdxZ--;
    ...
    for (int i = (int)sliceNumbers.size() - 1; i >= 0; i--)
    {
        ...
        //if (sliceNum == splitIdxZ - 1)
        //    DBG_DumpPoints("points0", vec);
        //if (sliceNum == splitIdxZ)
        //    DBG_DumpPoints("points1", vec);
        //if (sliceNum == splitIdxZ + 1)
        //    DBG_DumpPoints("points2", vec);
        ...
        //else
        //{
        //    stable_sort(vec.begin(), vec.end(), Point::IsSmallerY);
...
//TODO: uncomment this (without cam), and try to sort on z
...
#if 0 // Old version, computes too much
...
{
    // TODO; puth this somewhere else (not local)
    struct GreaterX
    {
        GreaterX(RC_FLOAT middleX)
        {
            mMiddleX = middleX;
        };
    ...
    //if (signY < /**/0)
    {
        sort(vec->begin(), vec->end(), GreaterY(middleY));
        if (signX >= /**/0)
            stable_sort(vec->begin(), vec->end(), GreaterX(middleX));
        if (signX < 0)
            stable_sort(vec->begin(), vec->end(), SmallerX(middleX));
    }
    //else
    //{
    //    if (signX >= /**/0)
    //        sort(vec->begin(), vec->end(), GreaterX(middleX));
    //    if (signX < 0)
                                       ...
                                       int *areaSign, RC_FLOAT *alignedCoord)
{
    ...
    for (int i = 0; i < mSlices.size(); i++)
    {
        ...
        // Avoid jumps from numSlices to 0 (1/2)
        //if (areaSign0 < 0)
        //    continue;
        if (areaSign0 < 0)
            allPositive = false;
    ...
    // Avoid jumps from numSlices to 0 (2/2)
    //
    // If not found, take the last slice (and compute all the necessary return values
    //
    //if (minIndex < 0)
        // Not found
