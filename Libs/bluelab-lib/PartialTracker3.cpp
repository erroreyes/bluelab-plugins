...
unsigned long PartialTracker3::Partial::mCurrentId = 0;
...
void
PartialTracker3::Partial::GenNewId()
{
    mId = mCurrentId++;
}
    
bool
PartialTracker3::Partial::FreqLess(const Partial &p1, const Partial &p2)
{
    return (p1.mFreq < p2.mFreq);
}

bool
PartialTracker3::Partial::AmpLess(const Partial &p1, const Partial &p2)
{
    return (p1.mAmpDB < p2.mAmpDB);
}

bool
PartialTracker3::Partial::IdLess(const Partial &p1, const Partial &p2)
{
    return (p1.mId < p2.mId);
}

bool
PartialTracker3::Partial::CookieLess(const Partial &p1, const Partial &p2)
{
    return (p1.mCookie < p2.mCookie);
}

PartialTracker3::PartialTracker3(int bufferSize, BL_FLOAT sampleRate,
...
PartialTracker3::~PartialTracker3()
{
    if (mFreqObj != NULL)
        delete mFreqObj;
}
...
void
PartialTracker3::SetThreshold(BL_FLOAT threshold)
{
...
#if 0 // ORIGINAL
void
PartialTracker3::SetData(const WDL_TypedBuf<BL_FLOAT> &magns,
                         ...
                         const WDL_TypedBuf<BL_FLOAT> &phases)
{
    ...
    WDL_TypedBuf<BL_FLOAT> magnsMel;
    BL_FLOAT hzPerBin = mSampleRate/mBufferSize;
    BLUtils::FreqsToMelNorm(&magnsMel, mCurrentMagns, (BL_FLOAT)hzPerBin, (BL_FLOAT)MIN_AMP_DB);
    mCurrentMagns = magnsMel;
...
void
PartialTracker3::DetectPartials()
{
...
void
PartialTracker3::ExtractNoiseEnvelope()
{
...
void
PartialTracker3::ExtractNoiseEnvelopeMax()
{
...
void
PartialTracker3::ExtractNoiseEnvelopeTrack()
{
    // If not partial detected, noise envelope is simply the input magns
    mNoiseEnvelope = mCurrentMagns;
...
void
PartialTracker3::ExtractNoiseEnvelopeSimple()
{
    vector<Partial> partials;
...
void
PartialTracker3::ProcessMusicalNoise(WDL_TypedBuf<BL_FLOAT> *noise)
{
...
void
PartialTracker3::ThresholdNoiseIsles(WDL_TypedBuf<BL_FLOAT> *noise)
{
    // Must choose bigger value than 1e-15
    // (otherwise the threshold won't work)
...
void
PartialTracker3::ZeroToNextNoiseMinimum(WDL_TypedBuf<BL_FLOAT> *noise)
{
    if (noise->GetSize() == 0)
        return;
...
void
PartialTracker3::SmoothNoiseEnvelope(WDL_TypedBuf<BL_FLOAT> *noise)
{
...
void
PartialTracker3::SmoothNoiseEnvelopeTime(WDL_TypedBuf<BL_FLOAT> *noise)
{
...
void
PartialTracker3::CutPartials(const vector<Partial> &partials,
...
void
PartialTracker3::KeepOnlyPartials(const vector<Partial> &partials,
                                  WDL_TypedBuf<BL_FLOAT> *magns)
{
...
void
PartialTracker3::CutPartialsMinEnv(WDL_TypedBuf<BL_FLOAT> *magns)
{
    // Detect the minima
...
void
PartialTracker3::FilterPartials()
{
#if !FILTER_PARTIALS
    mResult = mPartials[0];
#else // Filtered
...
bool
PartialTracker3::GetAlivePartials(vector<Partial> *partials)
{
    if (mPartials.empty())
        return false;
...
void
PartialTracker3::RemoveRealDeadPartials(vector<Partial> *partials)
{
...
void
PartialTracker3::GetPartials(vector<Partial> *partials)
{
    *partials = mResult;
...
void
PartialTracker3::ClearResult()
{
    mResult.clear();
}

void
PartialTracker3::GetNoiseEnvelope(WDL_TypedBuf<BL_FLOAT> *noiseEnv)
{
    *noiseEnv = mNoiseEnvelope;
}

void
PartialTracker3::GetHarmonicEnvelope(WDL_TypedBuf<BL_FLOAT> *harmoEnv)
{
    *harmoEnv = mHarmonicEnvelope;
}

void
PartialTracker3::SetMaxDetectFreq(BL_FLOAT maxFreq)
{
...
#if 0 // ORIGINAL
void
PartialTracker3::DetectPartials(const WDL_TypedBuf<BL_FLOAT> &magns,
                                const WDL_TypedBuf<BL_FLOAT> &phases,
...
void
PartialTracker3::SymetrisePartialFoot(int peakIndex,
...
bool
PartialTracker3::GluePartialBarbs(const WDL_TypedBuf<BL_FLOAT> &magns,
                                  vector<Partial> *partials)
{    
    vector<Partial> result;
    bool glued = false;
...
void
PartialTracker3::NarrowPartialFoot(const WDL_TypedBuf<BL_FLOAT> &magns,
                                   int peakIndex,
                                   int *leftIndex, int *rightIndex)
{
...
bool
PartialTracker3::DiscardFlatPartial(const WDL_TypedBuf<BL_FLOAT> &magns,
...
void
PartialTracker3::DiscardFlatPartials(const WDL_TypedBuf<BL_FLOAT> &magns,
                                     vector<Partial> *partials)
{
    vector<Partial> result;
    for (int i = 0; i < partials->size(); i++)
...
void
PartialTracker3::ApplyMinSpacingMel(vector<Partial> *partials)
{
    BL_FLOAT hzPerBin = mSampleRate/mBufferSize;
...
void
PartialTracker3::GlueTwinPartials(const WDL_TypedBuf<BL_FLOAT> &magns,
                                  vector<Partial> *partials)
{
    vector<Partial> result;
...
void
PartialTracker3::GlueTwinPartials(vector<Partial> *partials)
{
    vector<Partial> result;
...
void
PartialTracker3::SuppressZeroFreqPartials(vector<Partial> *partials)
{
    vector<Partial> result;
    for (int i = 0; i < partials->size(); i++)
    {
...
void
PartialTracker3::ThresholdPartialsAmp(vector<Partial> *partials)
{
    vector<Partial> result;
    for (int i = 0; i < partials->size(); i++)
    {
...
void
PartialTracker3::ThresholdPartialsPeakProminence(const WDL_TypedBuf<BL_FLOAT> &magns,
                                                 vector<Partial> *partials)
{
    vector<Partial> result;
    for (int i = 0; i < partials->size(); i++)
...
void
PartialTracker3::ThresholdPartialsPeakHeight(const WDL_TypedBuf<BL_FLOAT> &magns,
                                            vector<Partial> *partials)
{
    vector<Partial> result;
    for (int i = 0; i < partials->size(); i++)
...
void
PartialTracker3::ThresholdPartialsPeakHeightDb(const WDL_TypedBuf<BL_FLOAT> &magns,
                                               vector<Partial> *partials)
{
    vector<Partial> result;
    for (int i = 0; i < partials->size(); i++)
...
void
PartialTracker3::ThresholdPartialsPeakHeightPink(const WDL_TypedBuf<BL_FLOAT> &magns,
                                                 vector<Partial> *partials)
{
    vector<Partial> result;
    for (int i = 0; i < partials->size(); i++)
...
// Fixed version
BL_FLOAT
PartialTracker3::ComputePeakProminence(const WDL_TypedBuf<BL_FLOAT> &magns,
                                       int peakIndex, int leftIndex, int rightIndex)
{
    // Compute prominence
    //
...
BL_FLOAT
PartialTracker3::ComputePeakHeight(const WDL_TypedBuf<BL_FLOAT> &magns,
                                   int peakIndex, int leftIndex, int rightIndex)
{
    // Compute height
    //
...
BL_FLOAT
PartialTracker3::ComputePeakHeightDb(const WDL_TypedBuf<BL_FLOAT> &magns,
                                     int peakIndex, int leftIndex, int rightIndex,
...
BL_FLOAT
PartialTracker3::ComputePeakHigherFoot(const WDL_TypedBuf<BL_FLOAT> &magns,
                                       int leftIndex, int rightIndex)
{
...
BL_FLOAT
PartialTracker3::ComputePeakLowerFoot(const WDL_TypedBuf<BL_FLOAT> &magns,
                                      int leftIndex, int rightIndex)
{
    BL_FLOAT leftVal = magns.Get()[leftIndex];
    BL_FLOAT rightVal = magns.Get()[rightIndex];
...
void
PartialTracker3::ThresholdPartialsAmpSmooth(const WDL_TypedBuf<BL_FLOAT> &magns,
                                            vector<Partial> *partials)
{
...
//
void
PartialTracker3::ThresholdPartialsAmpAuto(const WDL_TypedBuf<BL_FLOAT> &magns,
                                          vector<Partial> *partials)
{
...
void
PartialTracker3::SuppressBarbs(vector<Partial> *partials)
{
...
// Filter
void
PartialTracker3::FilterPartials(vector<Partial> *result)
{
#define INF 1e15
...
BL_FLOAT
PartialTracker3::ComputePeakIndexAvg(const WDL_TypedBuf<BL_FLOAT> &magns,
                                     int leftIndex, int rightIndex)
{
...
//
BL_FLOAT
PartialTracker3::ComputePeakIndexParabola(const WDL_TypedBuf<BL_FLOAT> &magns,
                                          int peakIndex)
{
...
// Simple method
BL_FLOAT
PartialTracker3::ComputePeakAmpInterp(const WDL_TypedBuf<BL_FLOAT> &magns,
                                      BL_FLOAT peakFreq)
{
    BL_FLOAT hzPerBin = mSampleRate/mBufferSize;
...
BL_FLOAT
PartialTracker3::ComputePeakPhaseInterp(const WDL_TypedBuf<BL_FLOAT> &phases,
                                        BL_FLOAT peakFreq)
{
    BL_FLOAT hzPerBin = mSampleRate/mBufferSize;
    BL_FLOAT freqBin = peakFreq/hzPerBin;
...
BL_FLOAT
PartialTracker3::ComputePeakAmpInterpFreqObj(const WDL_TypedBuf<BL_FLOAT> &magns,
                                             BL_FLOAT peakFreq)
{
    int idx0 = 0;
...
// TODO: check / Mel !!
BL_FLOAT
PartialTracker3::GetFrequency(int binIndex)
{
...
int
PartialTracker3::FindPartialById(const vector<PartialTracker3::Partial> &partials,
                                 int idx)
{
    for (int i = 0; i < partials.size(); i++)
    {
...
void
PartialTracker3::AssociatePartials(const vector<PartialTracker3::Partial> &prevPartials,
                                   vector<PartialTracker3::Partial> *currentPartials,
                                   vector<PartialTracker3::Partial> *remainingPartials)
{
...
void
PartialTracker3::SmoothPartials(const vector<PartialTracker3::Partial> &prevPartials,
                                vector<PartialTracker3::Partial> *currentPartials)
{
    for (int i = 0; i < currentPartials->size(); i++)
    {
...
void
PartialTracker3::DBG_DumpPartials(const vector<Partial> &partials,
                                  int maxNumPartials)
{
...
void
PartialTracker3::DBG_DumpPartialsAmp(const vector<Partial> &partials,
...
void
PartialTracker3::DBG_DumpPartialsAmp(const char *fileName,
...
void
PartialTracker3::DBG_DumpPartialsBox(const vector<Partial> &partials,
                                     int bufferSize, BL_FLOAT sampleRate)
{
...
void
PartialTracker3::DBG_DumpPartialsBox(const char *fileName,
                                     const vector<Partial> &partials,
                                     int bufferSize, BL_FLOAT sampleRate)
{
#define NUM_MAGNS 1024
...
void
PartialTracker3::DBG_DumpPartials2(const vector<Partial> &partials)
{
...
// Manages Db
void
PartialTracker3::DBG_DumpPartials2(const char *fileName,
                                   const vector<Partial> &partials,
                                   int bufferSize, BL_FLOAT sampleRate)
{
    BL_FLOAT hzPerBin = sampleRate/bufferSize;
...
void
PartialTracker3::DBG_DumpPartials2Mel(const vector<Partial> &partials)
{
...
// (but better code)
void
PartialTracker3::DBG_DumpPartials2Mel(const char *fileName,
                                      const vector<Partial> &partials)
{
...
void
PartialTracker3::DBG_DumpPartials2MelHeight(const vector<Partial> &partials,
...
// (but better code)
void
PartialTracker3::DBG_DumpPartials2MelHeight(const char *fileName,
                                            const vector<Partial> &partials,
                                            const WDL_TypedBuf<BL_FLOAT> &magns)
{
    BL_FLOAT hzPerBin = mSampleRate/mBufferSize;
