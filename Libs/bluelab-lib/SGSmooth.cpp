...
// From http://www.ks.uiuc.edu/Research/vmd/mailing_list/vmd-l/att-13397/
// See also https://github.com/thatchristoph/vmd-cvs-github/tree/master/plugins/signalproc/src
...
//! default convergence
static const BL_FLOAT TINY_FLOAT = 1.0e-300;

//! comfortable array of BL_FLOATs
typedef std::vector<BL_FLOAT> float_vect;
...
static float_vect sg_derivative(const float_vect &v, const int w,
                                const int deg, const BL_FLOAT h=1.0);

// c-callable interface to for tcl plugin code
extern "C" BL_FLOAT *calc_sgsmooth(const int ndat, BL_FLOAT *input,
...
extern "C" BL_FLOAT *calc_sgsderiv(const int ndat, BL_FLOAT *input,
                                 const int window, const int order,
                                 const BL_FLOAT delta)
{
    float_vect in(ndat);
    int i;
...
public:
    ...
    float_mat(const size_t rows, const size_t cols, const BL_FLOAT def=0.0);
...
// constructor with sizes
float_mat::float_mat(const size_t rows,const size_t cols,const BL_FLOAT defval)
...
static int partial_pivot(float_mat &A, const size_t row, const size_t col,
                         float_vect &scale, int_vect &idx, BL_FLOAT tol)
{
    if (tol <= 0.0)
        tol = TINY_FLOAT;
    ...
    int pivot = row;
    BL_FLOAT piv_elem = std::fabs(A[idx[row]][col]) * scale[idx[row]];
    
    // loop over possible pivots below current
    int j;
    for (j = row + 1; j < A.nr_rows(); ++j) {
        
      const BL_FLOAT tmp = std::fabs(A[idx[j]][col]) * scale[idx[j]];
     ...
     * (e.g., {1,2, ... ,A.nr_rows()}). */
    static int lu_factorize(float_mat &A, int_vect &idx, BL_FLOAT tol=TINY_FLOAT)
    {
        if ( tol <= 0.0)
            tol = TINY_FLOAT;
    ...
    static float_vect sg_coeff(const float_vect &b, const size_t deg)
    {
        ...
        for (i = 0; i < rows; ++i) {
            for (j = 0; j < cols; ++j) {
	      A[i][j] = std::pow(BL_FLOAT(i), BL_FLOAT(j));
            }
        }
        ...
        for (i = 0; i < b.size(); ++i) {
            res[i] = c[0][0];
            for (j = 1; j <= deg; ++j) {
	      res[i] += c[j][0] * std::pow(BL_FLOAT(i), BL_FLOAT(j));
            }
        }
        return res;
    }
...
#pragma omp parallel for private(i,j) schedule(static)
    ...
    static float_vect lsqr_fprime(const float_vect &b, const int deg)
    {
        ...
        for (i = 0; i < b.size(); ++i) {
            res[i] = c[1][0];
            for (j = 1; j < deg; ++j) {
                res[i] += c[j + 1][0] * BL_FLOAT(j+1)
		  * std::pow(BL_FLOAT(i), BL_FLOAT(j));
            }
        }
        return res;
    }
    ...
    float_vect sg_derivative(const float_vect &v, const int width,
                             const int deg, const BL_FLOAT h)
    {
        float_vect res(v.size(), 0.0);
        if ((width < 1) || (deg < 1) || (v.size() < (2 * width + 2))) {
