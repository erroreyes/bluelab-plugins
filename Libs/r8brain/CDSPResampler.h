...
public:
	...
	CDSPResampler( const R8B_FLOAT SrcSampleRate, const R8B_FLOAT DstSampleRate,
		const int aMaxInLen, const R8B_FLOAT ReqTransBand = 2.0,
		const R8B_FLOAT ReqAtten = 206.91,
		...
		, LatencyFrac( 0.0 )
	{
		...
		for( i = 2; i <= 3; i++ )
		{
			...
			while( true )
			{
				const R8B_FLOAT NewSR = SrcSampleRate * ( i << c );
		...
		if( DstSampleRate * 2 > SrcSampleRate )
		{
			...
			const R8B_FLOAT NormFreq = ( DstSampleRate > SrcSampleRate ? 0.5 :
			...
			const R8B_FLOAT ThreshSampleRate = SrcSampleRate * 1.01;
			int c = 0;
			int div = 1;
			...
			const R8B_FLOAT SrcSampleRate2 = SrcSampleRate * 2.0;
			int tmp1;
			int tmp2;
			...
			if( c > 0 )
			{
				...
				const R8B_FLOAT tb = 100.0 * ( 1.0 - SrcSampleRate * div /
		...
		R8B_FLOAT CheckSR = DstSampleRate * 4.0;
		int c = 0;
		R8B_FLOAT FinGain = 1.0;
	...
	virtual int process( R8B_FLOAT* ip0, int l, R8B_FLOAT*& op0 )
	{
		R8BASSERT( l >= 0 );
	...
	{
		CFixedBuffer< R8B_FLOAT > Buf( MaxInLen );
		bool IsZero = false;
		...
		while( oplen > 0 )
		{
			...
			if( iplen == 0 )
			{
				...
				{
					IsZero = true;
					memset( p, 0, MaxInLen * sizeof( R8B_FLOAT ));
				}
			}
			else
			{
				...
				if( sizeof( Tin ) == sizeof( R8B_FLOAT ))
				{
					p = (R8B_FLOAT*) ip;
				}
				else
...
private:
	...
	int MaxInLen; ///< Maximal input length.
		///<
	CFixedBuffer< R8B_FLOAT > TmpBufAll; ///< Buffer containing both temporary
		///< buffers.
		///<
	R8B_FLOAT* TmpBufs[ 2 ]; ///< Temporary output buffers.
		///<
	...
	int CurMaxOutLen; ///< Current maximal output length.
		///<
	R8B_FLOAT LatencyFrac; ///< Current fractional latency. After object's
...
public:
	...
	CDSPResampler16( const R8B_FLOAT SrcSampleRate, const R8B_FLOAT DstSampleRate,
		const int aMaxInLen, const R8B_FLOAT ReqTransBand = 2.0 )
...
public:
	...
	CDSPResampler16IR( const R8B_FLOAT SrcSampleRate, const R8B_FLOAT DstSampleRate,
		const int aMaxInLen, const R8B_FLOAT ReqTransBand = 2.0 )
		: CDSPResampler( SrcSampleRate, DstSampleRate, aMaxInLen, ReqTransBand,
...
public:
	...
	CDSPResampler24( const R8B_FLOAT SrcSampleRate, const R8B_FLOAT DstSampleRate,
		const int aMaxInLen, const R8B_FLOAT ReqTransBand = 2.0 )
		: CDSPResampler( SrcSampleRate, DstSampleRate, aMaxInLen, ReqTransBand,
