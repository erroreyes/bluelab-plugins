...
public:
	...
	static R8B_FLOAT getLPMinTransBand()
	{
		return( 0.5 );
	}
	...
	static R8B_FLOAT getLPMaxTransBand()
	{
		return( 45.0 );
	}
	...
	static R8B_FLOAT getLPMinAtten()
	{
		return( 49.0 );
	}
	...
	static R8B_FLOAT getLPMaxAtten()
	{
		return( 218.0 );
	}
	...
	R8B_FLOAT getLatencyFrac() const
	{
		return( LatencyFrac );
	}
	...
	const R8B_FLOAT* getKernelBlock() const
	{
		return( KernelBlock );
	}
...
private:
	R8B_FLOAT ReqNormFreq; ///< Required normalized frequency, 0 to 1 inclusive.
		///<
	R8B_FLOAT ReqTransBand; ///< Required transition band in percent, as passed
		///< by the user.
		///<
	R8B_FLOAT ReqAtten; ///< Required stop-band attenuation in decibel, as passed
		///< by the user (positive value).
		///<
	EDSPFilterPhaseResponse ReqPhase; ///< Required filter's phase response.
		///<
	R8B_FLOAT ReqGain; ///< Required overall filter's gain.
		///<
	...
	int Latency; ///< Filter's latency in samples (integer part).
		///<
	R8B_FLOAT LatencyFrac; ///< Filter's latency in samples (fractional part).
		///<
	...
	int BlockLenBits; ///< Block length used to store *this FIR filter,
		...
		///< convolver.
		///<
	CFixedBuffer< R8B_FLOAT > KernelBlock; ///< FIR filter buffer, capacity
	...
	void buildLPFilter( const R8B_FLOAT* const ExtAttenCorrs )
	{
		const R8B_FLOAT tb = ReqTransBand * 0.01;
		R8B_FLOAT pwr;
		R8B_FLOAT fo1;
		R8B_FLOAT hl;
		R8B_FLOAT atten = -ReqAtten;
		...
		static const R8B_FLOAT AttenCorrMin = 49.0;
		static const R8B_FLOAT AttenCorrDiff = 176.25;
		int AttenCorr = (int) std::floor(( -atten - AttenCorrMin ) *
		...
		if( tb >= 0.25 )
		{
			static const R8B_FLOAT AttenCorrScale = 101.0;
		...
		if( tb >= 0.10 )
		{
			static const R8B_FLOAT AttenCorrScale = 210.0;
			static const signed char AttenCorrs[] = {
		...
		{
			static const R8B_FLOAT AttenCorrScale = 196.0;
			static const signed char AttenCorrs[] = {
		...
		pwr = 7.43932822146293e-8 * /*std::*/sqr( atten ) + 0.000102747434588003 *
        std::cos( 0.00785021930010397 * atten ) * std::cos( 0.633854318781239 +
			0.103208573657699 * atten ) - 0.00798132247867036 -
        0.000903555213543865 * atten - 0.0969365532127236 *
        std::exp(0.0779275237937911 * atten ) - 1.37304948662012e-5 * atten * std::cos(
		...
		if( pwr <= 0.067665322581 )
		{
			if( tb >= 0.25 )
			{
				hl = 2.6778150875894 / tb + 300.547590563091 * std::atan( std::atan(
					2.68959772209918 * pwr )) / ( 5.5099277187035 * tb - tb *
                                                 /*std::*/tanh( std::cos( asinh( atten ))));

				fo1 = 0.987205355829873 * tb + 1.00011788929851 *
                std::atan2(-0.321432067051302 - 6.19131357321578 * std::sqrt( pwr ),
                           hl + -1.14861472207245 / ( hl - 14.1821147585957 ) +
                           std::pow((R8B_FLOAT)0.9521145021664, std::pow( std::atan2( (R8B_FLOAT)1.12018764830637, tb ),
					(R8B_FLOAT)2.10988901686912 * hl - (R8B_FLOAT)20.9691278378345 )));
			}
			else
			if( tb >= 0.10 )
			{
				hl = ( 1.56688617018066 + 142.064321294568 * pwr +
                      0.00419441117131136 * std::cos( 243.633511747297 * pwr ) -
                      0.022953443903576 * atten - 0.026629568860284 * std::cos(
					127.715550622571 * pwr )) / tb;

				fo1 = 0.982299356642411 * tb + 0.999441744774215 *
                asinh((-0.361783054039583 - 5.80540593623676 * std::sqrt( pwr )) /
					hl );
			}
			else
			{
				hl = ( 2.45739657014937 + 269.183679500541 * pwr *
                      std::cos(5.73225668178813 + std::atan2( std::cosh( 0.988861169868941 -
					17.2201556280744 * pwr ), 1.08340138240431 * pwr ))) / tb;

				fo1 = 2.291956939 * tb + 0.01942450693 * /*std::*/sqr( tb ) * hl -
		...
		else
		{
			if( tb >= 0.25 )
			{
				hl = ( 1.50258368698213 + 158.556968859477 * asinh( pwr ) *
                      std::tanh( 57.9466246871383 * std::tanh( pwr )) -
					0.0105440479814834 * atten ) / tb;

				fo1 = 0.994024401639321 * tb +
                ( -0.236282717577215 - 6.8724924545387 * std::sqrt( std::sin( pwr ))) / hl;
			}
			else
			if( tb >= 0.10 )
			{
				hl = ( 1.50277377248945 + 158.222625721046 * asinh( pwr ) *
                      std::tanh( 1.02875299001715 + 42.072277322604 * pwr ) -
					0.0108380943845632 * atten ) / tb;

				fo1 = 0.992539376734551 * tb +
                ( -0.251747813037178 - 6.74159892452584 * std::sqrt( std::tanh( std::tanh( std::tan( pwr ))))) / hl;
			}
			else
			{
				hl = ( 1.15990238966306 * pwr - 5.02124037125213 *
                      /*std::*/sqr( pwr ) - 0.158676856669827 * atten *
                      std::cos( 1.1609073390614 * pwr - 6.33932586197475 * pwr * /*std::*/sqr( pwr ))) / tb;

				fo1 = 0.867344453126885 * tb + 0.052693817907757 * tb *
                std::log( pwr ) + 0.0895511178735932 * tb * std::atan( 59.7538527741309 *
					pwr ) - 0.0745653568081453 * pwr * tb;
			}
		}

		R8B_FLOAT WinParams[ 2 ];
		WinParams[ 0 ] = 125.0;
		WinParams[ 1 ] = pwr;
		...
		if( IsZeroPhase )
		{
			...
			memset( &KernelBlock[ sinc.fl2 + 1 ], 0,
				( BlockLen * 2 - KernelLen ) * sizeof( R8B_FLOAT ));
		}
		else
		{
...
public:
	...
	static CDSPFIRFilter& getLPFilter( const R8B_FLOAT ReqNormFreq,
		const R8B_FLOAT ReqTransBand, const R8B_FLOAT ReqAtten,
		const EDSPFilterPhaseResponse ReqPhase, const R8B_FLOAT ReqGain,
		const R8B_FLOAT* const AttenCorrs = NULL )
	{
