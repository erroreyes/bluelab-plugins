...
{
public:
	R8B_FLOAT Len2; ///< Required half filter kernel's length in samples (can be
	...
	union
	{
		struct
		{
			R8B_FLOAT Freq1; ///< Required corner circular frequency 1 [0; pi].
				///< Used only in the generateBand() function.
				///<
			R8B_FLOAT Freq2; ///< Required corner circular frequency 2 [0; pi].
		...
		struct
		{
			R8B_FLOAT FracDelay; ///< Fractional delay in the range [0; 1], used
	...
	typedef R8B_FLOAT( CDSPSincFilterGen :: *CWindowFunc )(); ///< Window
	...
	void initWindow( const EWindowFunctionType WinType = wftCosine,
		const R8B_FLOAT* const Params = NULL, const bool UsePower = false )
	{
		R8BASSERT( Len2 >= 2.0 );

		fl2 = (int) std::floor( Len2 );
		KernelLen = fl2 + fl2 + 1;
	...
	R8B_FLOAT calcWindowBlackmanNuttall()
	{
	...
	R8B_FLOAT calcWindowKaiser()
	{
		const R8B_FLOAT n = 1.0 - sqr( wn / Len2 + KaiserLen2Frac );
		wn++;
	...
	R8B_FLOAT calcWindowGaussian()
	{
		const R8B_FLOAT f = exp( -0.5 * sqr( wn / GaussianSigma +
			GaussianSigmaFrac ));
		...
		CWindowFunc wfunc = &CDSPSincFilterGen :: calcWindowBlackman )
	{
		...
		else
		{
			*op = pows(( *this.*wfunc )(), Power );
			...
			while( l > 0 )
			{
				const R8B_FLOAT v = pows(( *this.*wfunc )(), Power );
		...
		CWindowFunc wfunc = &CDSPSincFilterGen :: calcWindowBlackman )
	{
		...
		if( Power < 0.0 )
		{
			*op = ( Freq2 - Freq1 ) * ( *this.*wfunc )() / M_PI;
			...
			while( t <= fl2 )
			{
				const R8B_FLOAT v = ( f2.generate() - f1.generate() ) *
					( *this.*wfunc )() / t / M_PI;
	...
	void generateHilbert( T* op,
		CWindowFunc wfunc = &CDSPSincFilterGen :: calcWindowBlackman )
	{
		static const R8B_FLOAT fvalues[ 2 ] = { 0.0, 2.0 };
		op += fl2;
		T* op2 = op;
...
private:
	R8B_FLOAT Power; ///< The power factor used to raise the window function.
	...
	union
	{
		struct
		{
			R8B_FLOAT KaiserBeta; ///< Kaiser window function's "Beta"
				///< coefficient.
				///<
			R8B_FLOAT KaiserDiv; ///< Kaiser window function's divisor.
				///<
			R8B_FLOAT KaiserLen2Frac; ///< Equals FracDelay / Len2.
				///<
		};

		struct
		{
			R8B_FLOAT GaussianSigma; ///< Gaussian window function's "Sigma"
				///< coefficient.
				///<
			R8B_FLOAT GaussianSigmaFrac; ///< Equals FracDelay / GaussianSigma.
				///<
		};
	};
	...
	void setWindowKaiser( const R8B_FLOAT* Params, const bool UsePower,
	...
	void setWindowGaussian( const R8B_FLOAT* Params, const bool UsePower,
		const bool IsCentered )
	{
		wn = ( IsCentered ? 0 : -fl2 );
		...
		{
			GaussianSigma = clampr( std::fabs( Params[ 0 ]), 1e-1, 100.0 );
			Power = ( UsePower ? std::fabs( Params[ 1 ]) : -1.0 );
		}
	...
	void setWindow( const EWindowFunctionType WinType,
		const R8B_FLOAT* const Params, const bool UsePower,
		const bool IsCentered, const R8B_FLOAT UseFracDelay = 0.0 )
	{
		...
		if( WinType == wftCosine )
		{
			...
			{
				const R8B_FLOAT step1 = M_PI / Len2;
				w1.init( step1, M_PI * 0.5 - step1 * fl2 +
					step1 * FracDelay );

				const R8B_FLOAT step2 = M_2PI / Len2;
				w2.init( step2, M_PI * 0.5 - step2 * fl2 +
					step2 * FracDelay );

				const R8B_FLOAT step3 = M_3PI / Len2;
