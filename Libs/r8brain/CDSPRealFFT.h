...
public:
	...
	R8B_FLOAT getInvMulConst() const
	{
		return( InvMulConst );
	}
	...
	void forward( R8B_FLOAT* const p ) const
	{
	#if R8B_FLOATFFT
	...
	void inverse( R8B_FLOAT* const p ) const
	{
	#if R8B_IPP
	...
	void multiplyBlocks( const R8B_FLOAT* const aip1, const R8B_FLOAT* const aip2,
		R8B_FLOAT* const aop ) const
	{
	#if R8B_FLOATFFT
		...
		const R8B_FLOAT* const ip1 = aip1;
		const R8B_FLOAT* const ip2 = aip2;
		R8B_FLOAT* const op = aop;
	...
	void multiplyBlocks( const R8B_FLOAT* const aip, R8B_FLOAT* const aop ) const
	{
	#if R8B_FLOATFFT
	...
	void multiplyBlocksZ( const R8B_FLOAT* const aip, R8B_FLOAT* const aop ) const
	{
	#if R8B_FLOATFFT
	...
	void convertToZ( R8B_FLOAT* const ap ) const
	{
	#if R8B_FLOATFFT
...
private:
	...
	int Len; ///< Length of FFT block (number of real values).
		///<
	R8B_FLOAT InvMulConst; ///< Inverse FFT multiply constant.
		///<
	CDSPRealFFT* Next; ///< Next object in a singly-linked list.
		...
		CFixedBuffer< int > wi; ///< Working buffer (ints).
			///<
		CFixedBuffer< R8B_FLOAT > wd; ///< Working buffer (R8B_FLOATs).
			///<
	#endif // R8B_IPP
	...
	#endif // R8B_IPP
	{
		...
		wi.alloc( (int) ceil( 2.0 + sqrt( (R8B_FLOAT) ( Len >> 1 ))));
		wi[ 0 ] = 0;
		wd.alloc( Len >> 1 );
...
inline void calcMinPhaseTransform( R8B_FLOAT* const Kernel, const int KernelLen,
	const int LenMult = 2, const bool DoFinalMul = true,
	R8B_FLOAT* const DCGroupDelay = NULL )
{
	...
	CFixedBuffer< R8B_FLOAT > ip( Len );
	CFixedBuffer< R8B_FLOAT > ip2( Len2 + 1 );

	memcpy( &ip[ 0 ], Kernel, KernelLen * sizeof( R8B_FLOAT ));
	memset( &ip[ KernelLen ], 0, ( Len - KernelLen ) * sizeof( R8B_FLOAT ));
	...
	#else // R8B_FLOATFFT
		R8B_FLOAT* const aip = &ip[ 0 ];
		R8B_FLOAT* const aip2 = &ip2[ 0 ];
		const R8B_FLOAT nzbias = 1e-300;
	#endif // R8B_FLOATFFT

	aip2[ 0 ] = aip[ 0 ];
	aip[ 0 ] = std::log( std::fabs( aip[ 0 ]) + nzbias );
	aip2[ Len2 ] = aip[ 1 ];
	...
	const R8B_FLOAT m1 = ffto -> getInvMulConst();
	const R8B_FLOAT m2 = -m1;
	...
	for( i = 1; i < Len2; i++ )
	{
		aip[ i * 2 + 0 ] = std::cos( aip[ i * 2 + 1 ]) * aip2[ i ];
		aip[ i * 2 + 1 ] = std::sin( aip[ i * 2 + 1 ]) * aip2[ i ];
	}
	...
	{
		memcpy( &Kernel[ 0 ], &ip[ 0 ], KernelLen * sizeof( R8B_FLOAT ));
	}
