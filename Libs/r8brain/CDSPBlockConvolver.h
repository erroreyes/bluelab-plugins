...
public:
	...
	CDSPBlockConvolver( CDSPFIRFilter& aFilter, const int aUpFactor,
		const int aDownFactor, const R8B_FLOAT PrevLatency = 0.0,
	...
	virtual void clear()
	{
		memset( &PrevInput[ 0 ], 0, PrevInputLen * sizeof( R8B_FLOAT ));
		...
		memset( CurInput, 0, InputDelay * sizeof( R8B_FLOAT ));
	...
	virtual int process( R8B_FLOAT* ip, int l0, R8B_FLOAT*& op0 )
	{
		R8BASSERT( l0 >= 0 );
		...
		while( l > 0 )
		{
			...
			const int pil = (int) ( PrevInputLen * sizeof( R8B_FLOAT ));
			...
			if( DownShift > 0 )
			{
				...
				#else // R8B_FLOATFFT
					const R8B_FLOAT* const kb = Filter -> getKernelBlock();
					R8B_FLOAT* const p = CurInput;
				#endif // R8B_FLOATFFT
...
private:
	...
	int Latency; ///< Processing latency, in samples.
		///<
	R8B_FLOAT LatencyFrac; ///< Fractional latency, in samples, that is left in
		///< the output signal.
		///<
	...
	int InputDelay; ///< Additional input delay, in samples. Used to make the
		...
		///< and DownShift > 0.
		///<
	CFixedBuffer< R8B_FLOAT > WorkBlocks; ///< Previous input data, input and
		///< output data blocks, overall capacity = BlockLen2 * 2 +
		///< PrevInputLen. Used in the flip-flop manner.
		///<
	R8B_FLOAT* PrevInput; ///< Previous input data buffer, capacity = BlockLen.
		///<
	R8B_FLOAT* CurInput; ///< Input data buffer, capacity = BlockLen2.
		///<
	R8B_FLOAT* CurOutput; ///< Output data buffer, capacity = BlockLen2.
		///<
	...
	void copyUpsample( R8B_FLOAT*& ip0, R8B_FLOAT* op, int l0 )
	{
		int b = min( UpSkip, l0 );
	...
	void copyToOutput( int Offs, R8B_FLOAT*& op0, int b, int& l0 )
	{
		if( Offs < 0 )
		{
			if( Offs + b <= 0 )
		...
		else
		{
			if( df > 1 )
			{
				const R8B_FLOAT* ip = &CurOutput[ Offs + DownSkip ];
			...
			{
				memcpy( op0, &CurOutput[ Offs ], b * sizeof( R8B_FLOAT ));
				op0 += b;
				l0 += b;
			}
		}
