...
public:
	 ...
	 * -1 - use default.
	 * @param aElementSize The size of each filter's tap, in "R8B_FLOAT" values.
	...
	CDSPFracDelayFilterBank( const int aFilterFracs, const int aElementSize,
		const int aInterpPoints, const R8B_FLOAT aReqAtten, const bool aIsThird )
		...
		, RefCount( 1 )
	{
		...
		const R8B_FLOAT* const Params = getWinParams( ReqAtten, IsThird,
			FilterLen );
		...
		{
			FilterFracs = (int) std::ceil( 1.792462178761753 *
                                            std::exp( 0.033300466782047 * ReqAtten ));
		...
		{
			sinc.FracDelay = (R8B_FLOAT) ( FilterFracs - i ) / FilterFracs;
		...
		R8B_FLOAT* const TableEnd = Table + ( FilterFracs + 1 ) * FilterSize;
		p = Table;
	...
	static void roundReqAtten( R8B_FLOAT& att, const bool aIsThird )
	{
		int tmp;
	...
	const R8B_FLOAT& operator []( const int i ) const
	{
...
private:
	...
	int InterpPoints; ///< Interpolation points to use.
		///<
	R8B_FLOAT ReqAtten; ///< Filter's attentuation.
		///<
	bool IsThird; ///< "True" if one-third filter is in use.
		///<
	int FilterSize; ///< This constant specifies the "size" of a single filter
		///< in "R8B_FLOAT" elements.
		///<
	CFixedBuffer< R8B_FLOAT > Table; ///< The table of fractional delay filters
	...
	static const R8B_FLOAT* getWinParams( R8B_FLOAT& att, const bool aIsThird,
		int& fltlen )
	{
		const int CoeffCount = 13;
		static const R8B_FLOAT Coeffs[ CoeffCount ][ 3 ] = {
		...
		static const R8B_FLOAT CoeffsThird[ CoeffCountThird ][ 3 ] = {
...
public:
	...
	static CDSPFracDelayFilterBank& getFilterBank( const int aFilterFracs,
		...
		R8B_FLOAT ReqAtten, const bool IsThird, const bool IsStatic )
	{
...
inline bool findGCD( R8B_FLOAT l, R8B_FLOAT s, R8B_FLOAT& GCD )
{
	int it = 0;
...
inline bool getWholeStepping( const R8B_FLOAT SSampleRate,
	const R8B_FLOAT DSampleRate, int& ResInStep, int& ResOutStep )
{
	R8B_FLOAT GCD;
	...
	const R8B_FLOAT InStep0 = SSampleRate / GCD;
	ResInStep = (int) InStep0;
	const R8B_FLOAT OutStep0 = DSampleRate / GCD;
...
public:
	...
	CDSPFracInterpolator( const R8B_FLOAT aSrcSampleRate,
		const R8B_FLOAT aDstSampleRate, const R8B_FLOAT ReqAtten,
	...
	#endif // R8B_FASTTIMING
	{
		...
		InitFracPos -= Latency;
			...
			if( IsWhole )
			{
				...
				LatencyFrac = InitFracPos - (R8B_FLOAT) InitFracPosW / OutStep;
	...
	virtual int getMaxOutLen( const int MaxInLen ) const
	{
		...
		return( (int) std::ceil( MaxInLen * DstSampleRate / SrcSampleRate ) + 1 );
	}
...
private:
	...
	int flo; ///< Overrun length.
		///<
	R8B_FLOAT Buf[ BufLen + 29 ]; ///< The ring buffer, including overrun
		///< protection for maximal filter length.
		///<
	R8B_FLOAT SrcSampleRate; ///< Source sample rate.
		///<
	R8B_FLOAT DstSampleRate; ///< Destination sample rate.
		///<
	...
	int OutStep; ///< Output whole-number stepping (corresponds to filter bank
		///< size).
		///<
	R8B_FLOAT InitFracPos; ///< Initial fractional position, in samples, in the
		///< range [0; 1).
		///<
	...
	int Latency; ///< Initial latency that should be removed from the input.
		///<
	R8B_FLOAT LatencyFrac; ///< Left-over fractional latency.
		///<
	...
	int LatencyLeft; ///< Input latency left to remove.
		///<
	R8B_FLOAT InPosFrac; ///< Interpolation position (fractional part).
		///<
	...
	CDSPFracDelayFilterBank* FilterBank; ///< Filter bank in use, may be
		...
		///<
#if R8B_FASTTIMING
	R8B_FLOAT FracStep; ///< Fractional sample timing step.
#else // R8B_FASTTIMING
	int InCounter; ///< Interpolation step counter.
		///<
	int InPosInt; ///< Interpolation position (integer part).
		///<
	R8B_FLOAT InPosShift; ///< Interpolation position fractional shift.
		///<
#endif // R8B_FASTTIMING

	typedef R8B_FLOAT*( CDSPFracInterpolator :: *CConvolveFn )( R8B_FLOAT* op ); ///<
	...
	template< int fltlen >
	R8B_FLOAT* convolve0( R8B_FLOAT* op )
	{
		while( BufLeft > fl2 )
		{
			const R8B_FLOAT* const ftp = &(*FilterBank)[ InPosFracW ];
			const R8B_FLOAT* const rp = Buf + ReadPos;
			R8B_FLOAT s = 0.0;
			int i;
	...
	R8B_FLOAT* convolve2( R8B_FLOAT* op )
	{
		while( BufLeft > fl2 )
		{
			R8B_FLOAT x = InPosFrac * FilterBank -> getFilterFracs();
			const int fti = (int) x; // Function table index.
			x -= fti; // Coefficient for interpolation between
				// adjacent fractional delay filters.
			const R8B_FLOAT x2 = x * x;
			const R8B_FLOAT* const ftp = &(*FilterBank)[ fti ];
			const R8B_FLOAT* const rp = Buf + ReadPos;
			R8B_FLOAT s = 0.0;
			int ii = 0;
			int i;
			...
			*op = s;
			op++;
				...
				InCounter++;
				const R8B_FLOAT NextInPos = ( InCounter + InPosShift ) *
					SrcSampleRate / DstSampleRate;
