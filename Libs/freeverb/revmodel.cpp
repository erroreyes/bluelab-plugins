...
void revmodel::init(const FV_FLOAT sampleRate)
{
...
void revmodel::process(const FV_FLOAT in, FV_FLOAT &outputL, FV_FLOAT &outputR)
{
	FV_FLOAT outL,outR,input;
...
void revmodel::setroomsize(FV_FLOAT value)
{
	roomsize = ((value*scaleroom) + offsetroom); // * conversion;
	update();
}

FV_FLOAT revmodel::getroomsize()
{
//	return (roomsize/conversion-offsetroom)/scaleroom;
	return (roomsize-offsetroom)/scaleroom;
}

void revmodel::setdamp(FV_FLOAT value)
{
	damp = value*scaledamp/conversion;
    
    // FIX: avoid inf and nan if sample rate is > 44100Hz
    // (then conversion is > 1)
    // NIKO_FIX_FREEVERB
    // Niko comment
...
FV_FLOAT revmodel::getdamp()
{
//	return conversion * damp/scaledamp;
	return damp/scaledamp;
}

void revmodel::setwet(FV_FLOAT value)
{
	wet = value*scalewet;
	update();
}

FV_FLOAT revmodel::getwet()
{
	return wet/scalewet;
}

void revmodel::setdry(FV_FLOAT value)
{
	dry = value*scaledry;
}

FV_FLOAT revmodel::getdry()
{
	return dry/scaledry;
}

void revmodel::setwidth(FV_FLOAT value)
{
	width = value;
	update();
}

FV_FLOAT revmodel::getwidth()
{
	return width;
}

void revmodel::setmode(FV_FLOAT value)
{
	mode = value;
	update();
}

FV_FLOAT revmodel::getmode()
{
	if (mode >= freezemode)
		return 1;
	else
