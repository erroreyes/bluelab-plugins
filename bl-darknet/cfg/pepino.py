def CompileModel():
    ...
    pvconv = Conv2D(NVFiltPerc,(1025,1),activation = "relu",use_bias = False) #1
    phconv = Conv2D(NHFiltPerc,(1,6),activation = "relu",use_bias = False) # 3
    pflatter = Flatten()      
    pencoder = BatchNormalization()(stft_input) # 0
    pencoder = pvconv(pencoder) # 1
    pencoder = BatchNormalization()(pencoder) # 2
    pencoder = phconv(pencoder) #3
    pencoder = BatchNormalization()(pencoder) #4
    pencoder = pflatter(pencoder) #5
    ...
    hhconv = Conv2D(NHFiltHarm,(1,21),activation = "relu",use_bias = False) #7
    hvconv = Conv2D(NVFiltHarm,(82,1),activation = "relu",use_bias = False,strides = (41,1))    
    hflatter = Flatten()       
    hencoder = BatchNormalization()(stft_input) # 6
    hencoder = hhconv(hencoder) # 7
    hencoder = BatchNormalization()(hencoder) #8
    hencoder = hvconv(hencoder) # 9
    hencoder = BatchNormalization()(hencoder) #10
    hencoder = hflatter(hencoder) #11
    
    #Espacio Latente:
    latentspace = Concatenate()([hencoder,pencoder]) # 12
    latentspace = Dense(1024,activation = "relu",use_bias = False)(latentspace) #13
