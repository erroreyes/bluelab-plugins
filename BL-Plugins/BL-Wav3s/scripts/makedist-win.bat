...
set PT_GUID="14668960-316A-11EA-A14D-00505692C25A"

if %1 == 1 (echo Making BL-Wav3s Windows DEMO VERSION distribution ...) else (echo Making BL-Wav3s Windows FULL VERSION distribution ...)
...
REM - msbuild BL-Wav3s-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild BL-Wav3s.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild BL-Wav3s.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM move .\build-win\aax\Win32\Release\BL-Wav3s.aaxplugin .\build-win\aax\Win32\Release\BL-Wav3s.aaxplugin_unsigned

REM %WRAPTOOL% sign --verbose --account nicolas.suede --password "Shells222" --keyfile "..\..\..\Certificate\BlueLab.p12" --keypassword "FY@Somewh3re" --wcguid %PT_GUID% --in .\build-win\aax\Win32\Release\BL-Wav3s.aaxplugin_unsigned --out .\build-win\aax\Win32\Release\BL-Wav3s.aaxplugin

REM move .\build-win\aax\x64\Release\BL-Wav3s.aaxplugin .\build-win\aax\x64\Release\BL-Wav3s.aaxplugin_unsigned

REM %WRAPTOOL% sign --verbose --account nicolas.suede --password "Shells222" --keyfile "..\..\..\Certificate\BlueLab.p12" --keypassword "FY@Somewh3re" --wcguid %PT_GUID% --in .\build-win\aax\x64\Release\BL-Wav3s.aaxplugin_unsigned --out .\build-win\aax\x64\Release\BL-Wav3s.aaxplugin
...
REM /cc
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q ".\installer\BL-Wav3s.iss"
goto END-is

REM /cc
:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q ".\installer\BL-Wav3s.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "BL-Wav3s Installer" ".\installer\BL-Wav3s Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\BL-Wav3s Installer.exe" ".\installer\BL-Wav3s Demo Installer.exe"
REM -del ".\installer\BL-Wav3s Installer.exe"
REM -)
