...
# WIN INSTALLER
  ...
  for line in fileinput.input(projectpath + "/installer/" + config['BUNDLE_NAME'] + ".iss",inplace=1):
    ...
    if "OutputBaseFilename" in line:
      if demo:
        line="OutputBaseFilename=BL-Infra-v" + config['FULL_VER_STR'] + "[DEMO]-Installer\n"
      else:
        line="OutputBaseFilename=BL-Infra-v" + config['FULL_VER_STR'] + "-Installer\n"
    ...
    if "WelcomeLabel1" in line:
     if demo:
       line="WelcomeLabel1=Welcome to the BL-Infra Demo installer\n"
     else:
       line="WelcomeLabel1=Welcome to the BL-Infra installer\n"
       
    if "SetupWindowTitle" in line:
     if demo:
       line="SetupWindowTitle=BL-Infra [DEMO] installer\n"
     else:
       line="SetupWindowTitle=BL-Infra installer\n"
