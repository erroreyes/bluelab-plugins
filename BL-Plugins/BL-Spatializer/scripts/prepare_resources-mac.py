...
def main():
  ...
  # dummy file for getting resources path
  if os.path.exists(projectpath + "/resources/dummy.txt"):
    print "copying " + "dummy.txt" + " to " + dst
    shutil.copy(projectpath + "/resources/dummy.txt", dst)

  # dnn models for Rebalance
  if os.path.exists(projectpath + "/resources/models/"):
    models = os.listdir(projectpath + "/resources/models/")
    for model in models:
      print "copying " + model + " to " + dst
      shutil.copy(projectpath + "/resources/models/" + model, dst)

  # Hrtf for Spatializer
  if os.path.exists(projectpath + "/resources/hrtfs/KEMAR/"):
    models = os.listdir(projectpath + "/resources/hrtfs/KEMAR/")
    for model in models:
      print "copying " + model + " to " + dst
      shutil.copy(projectpath + "/resources/hrtfs/KEMAR/" + model, dst)
      
  # manual
  if os.path.exists(projectpath + "/manual/"):
    manuals = os.listdir(projectpath + "/manual/")
    for manual in manuals:
      print "copying " + manual + " to " + dst
      shutil.copy(projectpath + "/manual/" + manual, dst)
...
# AUDIOUNIT v3
  ...
  NSExtensionAttributes = dict(
                               AudioComponentBundle = "com.BlueLab.app." + config['BUNDLE_NAME'] + ".AUv3Framework",
                               AudioComponents = [{}]),
