#ifndef GHOST_TRACK2_H
#define GHOST_TRACK2_H
...
class GhostTrack2
{
 public:
    ...
    GhostTrack2(int bufferSize, BL_FLOAT sampleRate,
                Scale::Type yScale, int startMode = -1);
    virtual ~GhostTrack2();

    // Set to true if GhostTrack2 will be used to open audio files and store them
    void SetAppAPIHack(bool flag);
    ...
    bool GetDataSelection(BL_FLOAT dataSelection[4]);
    ...
    void RefreshData();
    void ResetSpectrogram(BL_FLOAT sampleRate);
    void RefreshSpectrogramView();
        
    //
    void ModeChanged(int prevMode);
    ...
    static void CopyTrackSelection(const GhostTrack2 &src, GhostTrack2 *dst);

    void SetColorMap(ColorMapFactory::ColorMap colormap);

    // Samples
    bool ContainsSamples() const;
    
    // Get all the file samples
    void GetSamples(vector<WDL_TypedBuf<BL_FLOAT> > *samples);
...
protected:
    ...
    // Selection
    friend class GhostCustomControl2;
...
protected:
 ...
 protected:
    friend class GhostSamplesToSpectro2;
    //
    int mBufferSize;
    ...
    GhostCustomDrawer2 *mCustomDrawer;
    GhostCustomDrawer2::State *mCustomDrawerState;
    
    GhostCustomControl2 *mCustomControl;
    ...
    GhostSamplesToSpectro2 *mSamplesToSpectro;
