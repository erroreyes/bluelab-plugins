...
MaskPredictor::Reset()
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        //mResamplers[i].SetRates(GetSampleRate(), SAMPLE_RATE);
...
MaskPredictor::ProcessInputSamplesPre(vector<WDL_TypedBuf<double> * > *ioSamples)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        //int numSamples0 = mResamplers[i].ResamplePrepare((*ioSamples)[i]->GetSize(),
        //                                                 1, &resampledAudio);
        ...
        outSamples.Resize(desiredSamples/*numSamples*/);
        //outSamples.Resize(numSamples0);
        int numResampled = mResamplers[i].ResampleOut(outSamples.Get(),
                                                      (*ioSamples)[i]->GetSize(),
                                                      outSamples.GetSize()/*numSamples*/, 1);
...
MaskPredictor::GetMaskOther(WDL_TypedBuf<double> *maskOther)
{
    *maskOther = mMaskOther;
    ...
    // TEST SUNDAY: avoid pumping effect when extracting voice ! (or removing it)
    //Utils::FillAllZero(maskOther);
}
...
MaskPredictor::InitResamplers()
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        // set input and output samplerates
        //mResamplers[i].SetRates(mSampleRate, SAMPLE_RATE);
        mResamplers[i].SetRates(mPlugSampleRate, SAMPLE_RATE);
    }
}
#endif
...
RebalanceProcessFftObj::ProcessSamplesPost(WDL_TypedBuf<double> *ioBuffer)
{
    ...
    WDL_ResampleSample *resampledAudio = NULL;
    //int desiredSamples = ioBuffer->GetSize();
    ...
    //                                             numSamples, outSamples.GetSize(), 1);
    int numResampled = mResamplerOut.ResampleOut(outSamples.Get(),
                                                 //numSamples,
                                                 ioBuffer->GetSize(),
...
RebalanceProcessFftObj::InitResamplers()
{
    ...
    //mResamplerOut.SetFeedMode(true);
