...
# WIN INSTALLER
  ...
  for line in fileinput.input(projectpath + "/installer/" + config['BUNDLE_NAME'] + ".iss",inplace=1):
    ...
    if "OutputBaseFilename" in line:
      if demo:
        line="OutputBaseFilename=BL-PitchShift-v" + config['FULL_VER_STR'] + "[DEMO]-Installer\n"
      else:
        line="OutputBaseFilename=BL-PitchShift-v" + config['FULL_VER_STR'] + "-Installer\n"
    ...
    if "WelcomeLabel1" in line:
     if demo:
       line="WelcomeLabel1=Welcome to the BL-PitchShift Demo installer\n"
     else:
       line="WelcomeLabel1=Welcome to the BL-PitchShift installer\n"
       
    if "SetupWindowTitle" in line:
     if demo:
       line="SetupWindowTitle=BL-PitchShift [DEMO] installer\n"
     else:
       line="SetupWindowTitle=BL-PitchShift installer\n"
