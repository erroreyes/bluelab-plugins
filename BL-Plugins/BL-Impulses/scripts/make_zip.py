...
def  main():
  ...
  installer = "\installer\BL-Impulses-v" + FULLVERSIONSTR + "-Installer.exe"
   
  if demo:
    installer = "\installer\BL-Impulses-v" + FULLVERSIONSTR + "[DEMO]" + "-Installer.exe"
  ...
  FILES_TO_ZIP = [
    ...
    projectpath + "\manual\BL-Impulses_manual-EN.pdf" 
    projectpath + "\Examples\*" 
  ]

  ZIPNAME = "BL-Impulses-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "BL-Impulses-v" + FULLVERSIONSTR + "[DEMO]-win.zip"
